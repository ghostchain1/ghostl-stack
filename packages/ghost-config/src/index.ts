export const GHOST_SITES = Object.freeze({
  ...
  wallet: Object.freeze({
    ...
    domain: "apps.ghostchain.cloud",
    url: "https://apps.ghostchain.cloud/wallet",
...
export const GHOST_SITE_DIRECTORY = Object.freeze([
  GHOST_SITES.main,
...
export const GHOST_PUBLIC_NAV_LINKS = Object.freeze([
...
export const GHOST_PUBLIC_FOOTER_COLUMNS = Object.freeze([
  Object.freeze({
    title: "Platform",
    links: Object.freeze([
...
export const GHOST_SERVICES = Object.freeze({
  api: Object.freeze({
...
export const GHOST_GNS = Object.freeze({
...
export const GHOST_OWNED_DOMAINS = Object.freeze([
  Object.freeze({ domain: "ghostchain.cloud", gnsEnabled: true, status: "healthy" }),
  Object.freeze({ domain: "ghostchain.info", gnsEnabled: true, status: "healthy" }),
  Object.freeze({ domain: "ghostchain.life", gnsEnabled: true, status: "healthy" }),
  Object.freeze({ domain: "ghostchain.live", gnsEnabled: true, status: "healthy" }),
  Object.freeze({ domain: "ghostchain.online", gnsEnabled: true, status: "healthy" }),
  Object.freeze({ domain: "ghostchain.space", gnsEnabled: true, status: "healthy" }),
  Object.freeze({ domain: "ghostchain.store", gnsEnabled: true, status: "healthy" }),
  Object.freeze({ domain: "ghostchain.world", gnsEnabled: true, status: "healthy" }),
  Object.freeze({ domain: "ghostchainlink.com", gnsEnabled: false, status: "healthy" }),
  Object.freeze({ domain: "ghostchainsolutions.com", gnsEnabled: false, status: "healthy" }),
  Object.freeze({ domain: "ghostschain.com", gnsEnabled: false, status: "healthy" }),
]);

export const GHOST_DNS_ZONES = Object.freeze(
  Array.from(
    [
      GHOST_SITES.main,
      GHOST_SITES.apps,
      ...
      const siteStatus = "status" in site ? site.status : undefined;
      const current =
      ...
      zones.set(
        ...
        Object.freeze({
          domain: site.domain,
          status: siteStatus || current.status || "healthy",
      ...
      return zones;
    }, new Map<string, { domain: string; status: string; gnsEnabled: boolean; recordCount: number; ttl: number }>())
      .values()
  ).sort((left, right) => left.domain.localeCompare(right.domain))
);

export function getGhostSite(siteKey: keyof typeof GHOST_SITES) {
...
export function getGhostSiteUrl(siteKey: keyof typeof GHOST_SITES) {
  return getGhostSite(siteKey).url;
}

export function getGhostService(serviceKey: keyof typeof GHOST_SERVICES) {
...
export function createPublicSiteEnv(
  siteKey: keyof typeof GHOST_SITES,
...
export function createPublicSiteNextConfig(
  siteKey: keyof typeof GHOST_SITES,
  options: Record<string, unknown> & { env?: EnvRecord } = {}
) {
  ...
  return {
    ...
    ...rest,
    env: {
      ...createPublicSiteEnv(siteKey, getDefaultEnv(), env),
    },
  };
}
