...
export async function run(ctx: CLIContext): Promise<void> {
  ...
  switch (sub) {
    case "resolve": {
      ...
      if (!name) { log.error("Usage: ghost gns resolve <name>"); process.exit(1); }
      log.info(`Resolving: ${name}`);
      try {
        const resp = await callGhostBrain(cfg.ghostbrainUrl, "/gns/resolve", { name });
        console.log(JSON.stringify(resp, null, 2));
      } catch (err) {
        log.error(`Resolve failed: ${(err as Error).message}`);
        process.exit(1);
      }
      break;
    }
    ...
    case "register": {
      ...
      try {
        ...
        log.error(`Register failed: ${(err as Error).message}`);
        process.exit(1);
      }
      break;
    }
