...
async function checkBinary(bin: string, arg = "--version"): Promise<Check> {
  try {
...
async function checkRpc(label: string, url: string): Promise<Check> {
  try {
    ...
    return { name: `RPC ${label}`, ok: false, detail: (err as Error).message };
  }
}
