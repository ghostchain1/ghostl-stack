...
async function layerStatus(label: string, url: string) {
  try {
    const [blockHex, peersHex, syncingRaw] = await Promise.all([
      rpcRequest(url, "eth_blockNumber") as Promise<string>,
      rpcRequest(url, "net_peerCount")   as Promise<string>,
      rpcRequest(url, "eth_syncing"),
    ]);
    const block   = parseInt(blockHex, 16);
    const peers   = parseInt(peersHex as string, 16);
    const syncing = syncingRaw !== false;
    return { label, block, peers, syncing, healthy: true, error: undefined as string | undefined };
  } catch (err) {
    return { label, block: 0, peers: 0, syncing: false, healthy: false, error: (err as Error).message };
  }
}
  ...
  switch (sub) {
    case "status": {
      ...
      for (const s of [l1, l2, l3]) {
        ...
        console.log(
          `${icon} ${s.label.padEnd(4)} block=${s.block} peers=${s.peers} syncing=${s.syncing}${s.error ? ` (${s.error})` : ""}`,
        );
      }
      break;
    }
    ...
    case "switch": {
      const layer = ctx.args[2] as "l1" | "l2" | "l3" | undefined;
    ...
    case "ping": {
      ...
      try {
        ...
        log.error(`Ping failed: ${(err as Error).message}`);
        process.exit(1);
      }
      break;
    }
