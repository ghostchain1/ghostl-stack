...
export class GhostValidatorAutoRepair {
  private readonly health = new GhostValidatorHealth();

  async repair(rpcUrl: string, minPeers: number): Promise<RepairResult> {
  ...
  private async dockerRestartValidator(): Promise<boolean> {
    try {
  ...
  private async addBootnodesViaAdmin(rpcUrl: string): Promise<void> {
