...
export class GhostProjectGenerator {
    tpl = new GhostContractTemplate();
    async create(dir, opts = {}) {
        ...
        // Sample contract
        writeFileSync(join(dir, `contracts/src/${name}.sol`), this.tpl.generate(name), "utf8");
        // Sample test
        writeFileSync(join(dir, `contracts/test/${name}.t.sol`), this.tpl.generateTest(name), "utf8");
        // Deploy script
        writeFileSync(join(dir, `contracts/script/Deploy${name}.s.sol`), this.tpl.generateScript(name), "utf8");
        // .gitignore
