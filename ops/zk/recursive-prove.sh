...
CIRCUIT_PATH="$ROOT_DIR/ops/zk/circuit/recursive_aggregate.circom"
BUILD_DIR="$ROOT_DIR/ops/zk/build-recursive"
...
usage() {
  cat <<'USAGE'
Usage: recursive-prove.sh --input <path> --out-proof <path> --out-vkey <path>
USAGE
}
...
ZKEY_PATH="$KEYS_DIR/recursive_final.zkey"
VKEY_PATH="$OUT_VKEY"
...
if [[ ! -f "$ZKEY_PATH" ]]; then
  ...
  snarkjs groth16 setup "$BUILD_DIR/recursive_aggregate.r1cs" "$PTAU_PATH" "$BUILD_DIR/recursive_0000.zkey"
  snarkjs zkey contribute "$BUILD_DIR/recursive_0000.zkey" "$ZKEY_PATH" --name "ghost-recursive" --entropy "$ENTROPY"
fi

snarkjs zkey export verificationkey "$ZKEY_PATH" "$VKEY_PATH"

node "$BUILD_DIR/recursive_aggregate_js/generate_witness.js" "$INPUT_PATH" "$BUILD_DIR/witness.wtns"
