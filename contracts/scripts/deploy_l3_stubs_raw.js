...
async function main() {
  ...
  const l2ooFactory = new ethers.ContractFactory(l2ooArtifact.abi, l2ooArtifact.bytecode, signer);
  const dgfFactory = new ethers.ContractFactory(dgfArtifact.abi, dgfArtifact.bytecode, signer);

  const l2ooTxReq = await l2ooFactory.getDeployTransaction(0);
  l2ooTxReq.nonce = nonce;
  const l2ooAddr = ethers.getCreateAddress({ from: signer.address, nonce });
  ...
  const dgfAddr = ethers.getCreateAddress({ from: signer.address, nonce: nonce + 1 });
