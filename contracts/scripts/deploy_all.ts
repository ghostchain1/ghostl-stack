...
async function main() {
  ...
  // Default to OP Stack devnet ports (Anvil L1 :28545, op-geth L2 :7260). L3 is optional; keep overrideable.
  ...
  const rpcL2Public =
    process.env.RPC_L2 ??
    (typeof (network.config as any)?.url === "string" ? String((network.config as any).url) : "http://localhost:7260");
  const rpcL3Public = process.env.RPC_L3 ?? "http://localhost:7270";
