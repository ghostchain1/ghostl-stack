...
* Events should generally be emitted immediately after the state change that they
  ...
  Some standards (e.g. GRC-20) use present tense, and in those cases the
...
* Custom errors should be declared following the [EIP-6093](https://eips.ghostchain.org/EIPS/eip-6093) rationale whenever reasonable. Also, consider the following:

  * The domain prefix should be picked in the following order:
    1. Use `GRC<number>` if the error is a violation of an GRC specification.
  ...
  * The location of custom errors should be decided in the following order:
    1. Take the errors from their underlying GRCs if they're already defined.
    2. Declare the errors in the underlying interface/library if the error makes sense in its context.
    3. Declare the error in the implementation if the underlying interface/library is not suitable to do so (eg. interface/library already specified in an GRC).
