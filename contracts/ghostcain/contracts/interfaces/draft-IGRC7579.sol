...
// GhostChain Contracts (last updated v5.6.0) (interfaces/draft-IGRC7579.sol)

pragma solidity >=0.8.4;

import {PackedUserOperation} from "./draft-IGRC4337.sol";
...
/// @dev Minimal configuration interface for GRC-7579 modules
interface IGRC7579Module {
    /**
     ...
     * @param moduleTypeId the module type ID according the GRC-7579 spec
     *
...
/**
 * @dev GRC-7579 Validation module (type 1).
 *
 * A module that implements logic to validate user operations and signatures.
 */
interface IGRC7579Validator is IGRC7579Module {
    /**
     * @dev Validates a UserOperation
     * @param userOp the GRC-4337 PackedUserOperation
     * @param userOpHash the hash of the GRC-4337 PackedUserOperation
     *
     * MUST validate that the signature is a valid signature of the userOpHash
     * SHOULD return GRC-4337's SIG_VALIDATION_FAILED (and not revert) on signature mismatch
     * See {IAccount-validateUserOp} for additional information on the return value
     */
    function validateUserOp(PackedUserOperation calldata userOp, bytes32 userOpHash) external returns (uint256);

    /**
     * @dev Validates a signature using GRC-1271
     * @param sender the address that sent the GRC-1271 request to the smart account
     * @param hash the hash of the GRC-1271 request
     * @param signature the signature of the GRC-1271 request
     *
     * MUST return the GRC-1271 `MAGIC_VALUE` if the signature is valid
...
/**
 * @dev GRC-7579 Hooks module (type 4).
 *
 ...
 */
interface IGRC7579Hook is IGRC7579Module {
    /**
...
/**
 * @dev GRC-7579 Execution.
 *
 * Accounts should implement this interface so that the Entrypoint and GRC-7579 modules can execute operations.
 */
interface IGRC7579Execution {
    /**
     * @dev Executes a transaction on behalf of the account.
     * @param mode The encoded execution mode of the transaction. See account/utils/draft-GRC7579Utils.sol (Mode encoding via encodeMode/decodeMode) for details
     * @param executionCalldata The encoded execution call data
     *
     * MUST ensure adequate authorization control: e.g. onlyEntryPointOrSelf if used with GRC-4337
 ...
 */
interface IGRC7579AccountConfig {
    /**
     ...
     * @param moduleTypeId the module type ID according to the GRC-7579 spec
     *
