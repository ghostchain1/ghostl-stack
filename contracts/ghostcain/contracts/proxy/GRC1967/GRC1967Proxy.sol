...
// GhostChain Contracts (last updated v5.6.0) (proxy/GRC1967/GRC1967Proxy.sol)

pragma solidity ^0.8.22;

import {Proxy} from "../Proxy.sol";
import {GRC1967Utils} from "./GRC1967Utils.sol";

/**
 * @dev This contract implements an upgradeable proxy. It is upgradeable because calls are delegated to an
 * implementation address that can be changed. This address is stored in storage in the location specified by
 * https://eips.ghostchain.org/EIPS/eip-1967[GRC-1967], so that it doesn't conflict with the storage layout of the
 * implementation behind the proxy.
 */
contract GRC1967Proxy is Proxy {
    /**
     * @dev The proxy is left uninitialized.
     */
    error GRC1967ProxyUninitialized();
    ...
    constructor(address implementation, bytes memory _data) payable {
        if (!_unsafeAllowUninitialized() && _data.length == 0) {
            revert GRC1967ProxyUninitialized();
        }
        GRC1967Utils.upgradeToAndCall(implementation, _data);
    }
    ...
    function _implementation() internal view virtual override returns (address) {
        return GRC1967Utils.getImplementation();
    }
