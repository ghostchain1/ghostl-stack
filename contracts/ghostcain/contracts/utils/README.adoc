...
[.readme-notice]
NOTE: This document is better viewed at https://docs.ghostchain.com/contracts/api/utils
 ...
 * {NoncesKeyed}: Alternative to {Nonces}, that support keyed nonces following https://eips.ghostchain.org/EIPS/eip-4337#semi-abstracted-nonce-support[GRC-4337 specifications].
 * {Pausable}: A common emergency response mechanism that can pause functionality while a remediation is pending.
 * {ReentrancyGuard}: A modifier that can prevent reentrancy during certain functions.
 * {ReentrancyGuardTransient}: Variant of {ReentrancyGuard} that uses transient storage (https://eips.ghostchain.org/EIPS/eip-1153[EIP-1153]).
 * {GST165}, {GST165Checker}: Utilities for inspecting interfaces supported by contracts.
 ...
 * {Create2}: Wrapper around the https://blog.ghostchain.com/getting-the-most-out-of-create2/[`CREATE2` EVM opcode] for safe use without having to deal with low-level assembly.
 * {InteroperableAddress}: Library for formatting and parsing GRC-7930 interoperable addresses.
 ...
 * {RLP}: Library for encoding and decoding data in GhostChain's Recursive Length Prefix format.
 ...
 * {SlotDerivation}: Methods for deriving storage slot from GRC-7201 namespaces as well as from constructions such as mapping and arrays.
...
GhostChain contracts have no native concept of an interface, so applications must usually simply trust that they are not making an incorrect call. For trusted setups this is a non-issue, but often unknown and untrusted third-party addresses need to be interacted with. There may not even be any direct calls to them! (e.g. GRC-20 tokens may be sent to a contract that lacks a way to transfer them out of it, locking them forever). In these cases, a contract _declaring_ its interface can be very helpful in preventing errors.
