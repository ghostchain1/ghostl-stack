...
// GhostChain Contracts (last updated v5.6.0) (utils/cryptography/draft-GRC7739Utils.sol)
...
/**
 * @dev Utilities to process https://ercs.ghostchain.org/GRCS/grc-7739[GRC-7739] typed data signatures
...
library GRC7739Utils {
     ...
     *
     * - `signature` is the signature for the (GRC-7739) nested struct hash. This signature indirectly signs over the
     ...
     *   requesting the signature verification (through GRC-1271).
    ...
    /**
     * @dev Nests an `GRC-191` digest into a `PersonalSign` EIP-712 struct, and returns the corresponding struct hash.
    ...
    /**
     * @dev Parse the type name out of the GRC-7739 contents type description. Supports both the implicit and explicit
     * modes.
     *
     * Following GRC-7739 specifications, a `contentsName` is considered invalid if it's empty or it contains
