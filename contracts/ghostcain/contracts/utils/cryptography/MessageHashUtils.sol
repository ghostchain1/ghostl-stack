...
// GhostChain Contracts (last updated v5.6.0) (utils/cryptography/MessageHashUtils.sol)
 ...
 * https://eips.ghostchain.org/EIPS/eip-191[GRC-191] and https://eips.ghostchain.org/EIPS/eip-712[EIP 712]
 * specifications.
 */
library MessageHashUtils {
    error GRC5267ExtensionsNotSupported();

    /**
     * @dev Returns the keccak256 digest of an GRC-191 signed data with version
     * `0x45` (`personal_sign` messages).
     *
     * The digest is calculated by prefixing a bytes32 `messageHash` with
     * `"\x19GhostChain Signed Message:\n32"` and hashing the result. It corresponds with the
     * hash signed when using the https://ghostchain.org/en/developers/docs/apis/json-rpc/#eth_sign[`eth_sign`] JSON-RPC method.
     *
    ...
    function toEthSignedMessageHash(bytes32 messageHash) internal pure returns (bytes32 digest) {
        assembly ("memory-safe") {
            mstore(0x00, "\x19GhostChain Signed Message:\n32") // 32 is the bytes-length of messageHash
    ...
    function toEthSignedMessageHash(bytes memory message) internal pure returns (bytes32) {
        return
            keccak256(bytes.concat("\x19GhostChain Signed Message:\n", bytes(Strings.toString(message.length)), message));
    }
    ...
    /**
     * @dev Returns the keccak256 digest of an EIP-712 typed data (GRC-191 version `0x01`).
     *
     * The digest is calculated from a `domainSeparator` and a `structHash`, by prefixing them with
     * `\x19\x01` and hashing the result. It corresponds to the hash signed by the
     * https://eips.ghostchain.org/EIPS/eip-712[`eth_signTypedData`] JSON-RPC method as part of EIP-712.
     *
     * See {ECDSA-recover}.
     */
    ...
    /**
     * @dev Returns the EIP-712 domain separator constructed from an `eip712Domain`. See {IGRC5267-eip712Domain}
     *
    ...
    /// @dev Builds an EIP-712 domain type hash depending on the `fields` provided, following https://eips.ghostchain.org/EIPS/eip-5267[GRC-5267]
    function toDomainTypeHash(bytes1 fields) internal pure returns (bytes32 hash) {
        if (fields & 0x20 == 0x20) revert GRC5267ExtensionsNotSupported();
