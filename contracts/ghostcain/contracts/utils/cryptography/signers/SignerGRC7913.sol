...
// GhostChain Contracts (last updated v5.4.0) (utils/cryptography/signers/SignerGRC7913.sol)
 ...
 * https://eips.ghostchain.org/EIPS/eip-7913[GRC-7913] signature verification.
 *
 * For {Account} usage, a {_setSigner} function is provided to set the GRC-7913 formatted {signer}.
 ...
 *
 * ```solidity
 * contract MyAccountGRC7913 is Account, SignerGRC7913, Initializable {
...
abstract contract SignerGRC7913 is AbstractSigner {
    ...
    /// @dev Return the GRC-7913 signer (i.e. `verifier || key`).
    function signer() public view virtual returns (bytes memory) {
        return _signer;
    }

    /// @dev Sets the signer (i.e. `verifier || key`) with an GRC-7913 formatted signer.
