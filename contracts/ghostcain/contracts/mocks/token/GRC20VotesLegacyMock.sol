 ...
 */
abstract contract GRC20VotesLegacyMock is IVotes, GRC20Permit {
    struct Checkpoint {
        uint32 fromBlock;
        uint224 votes;
    }
    ...
    function getPastVotes(address account, uint256 blockNumber) public view virtual returns (uint256) {
        require(blockNumber < block.number, "GRC20Votes: block not yet mined");
    ...
    function delegateBySig(
        ...
        require(block.timestamp <= expiry, "GRC20Votes: signature expired");
        ...
        require(nonce == _useNonce(signer), "GRC20Votes: invalid nonce");
    ...
    function _update(address from, address to, uint256 amount) internal virtual override {
        ...
        if (from == address(0)) {
            require(totalSupply() <= _maxSupply(), "GRC20Votes: total supply risks overflowing votes");
