...
contract GRC1155ReceiverMock is GST165, IGRC1155Receiver {
    enum RevertType {
        None,
    ...
    function onGRC1155Received(
        ...
        if (_error == RevertType.RevertWithoutMessage) {
            ...
            revert("GRC1155ReceiverMock: reverting on receive");
    ...
    function onGRC1155BatchReceived(
        ...
        if (_error == RevertType.RevertWithoutMessage) {
            ...
            revert("GRC1155ReceiverMock: reverting on batch receive");
        } else if (_error == RevertType.RevertWithCustomError) {
            revert CustomError(_recRetval);
        } else if (_error == RevertType.Panic) {
            uint256 a = uint256(0) / uint256(0);
