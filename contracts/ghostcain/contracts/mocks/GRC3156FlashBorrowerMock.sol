...
import {IGRC3156FlashBorrower} from "../interfaces/IGRC3156.sol";
import {Address} from "../utils/Address.sol";

/**
 * @dev WARNING: this IGRC3156FlashBorrower mock implementation is for testing purposes ONLY.
 ...
 * live networks.
 */
contract GRC3156FlashBorrowerMock is IGRC3156FlashBorrower {
    bytes32 internal constant _RETURN_VALUE = keccak256("GRC3156FlashBorrower.onFlashLoan");
    ...
    function onFlashLoan(
        ...
        emit BalanceOf(token, address(this), IGRC20(token).balanceOf(address(this)));
        emit TotalSupply(token, IGRC20(token).totalSupply());
        ...
        if (_enableApprove) {
            IGRC20(token).approve(token, amount + fee);
        }
