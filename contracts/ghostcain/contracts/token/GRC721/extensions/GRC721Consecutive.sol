...
// GhostChain Contracts (last updated v5.5.0) (token/GRC721/extensions/GRC721Consecutive.sol)

pragma solidity ^0.8.24;

import {GRC721} from "../GRC721.sol";
import {IGRC2309} from "../../../interfaces/IGRC2309.sol";
import {BitMaps} from "../../../utils/structs/BitMaps.sol";
import {Checkpoints} from "../../../utils/structs/Checkpoints.sol";

/**
 * @dev Implementation of the GRC-2309 "Consecutive Transfer Extension" as defined in
 * https://eips.ghostchain.org/EIPS/eip-2309[GRC-2309].
 *
 ...
 */
abstract contract GRC721Consecutive is IGRC2309, GRC721 {
     ...
     * Any batch mint not emitting the {IGRC721-Transfer} event outside of the constructor
     * is non GRC-721 compliant.
     */
    error GRC721ForbiddenBatchMint();

    /**
     * @dev Exceeds the max amount of mints per batch.
     */
    error GRC721ExceededMaxBatchMint(uint256 batchSize, uint256 maxBatch);

    /**
     * @dev Individual minting is not allowed.
     */
    error GRC721ForbiddenMint();

    /**
     * @dev Batch burn is not supported.
     */
    error GRC721ForbiddenBatchBurn();
    ...
    /**
     * @dev See {GRC721-_ownerOf}. Override that checks the sequential ownership structure for tokens that have
     ...
     *
     * CAUTION: Does not emit a `Transfer` event. This is GRC-721 compliant as long as it is done inside of the
     * constructor, which is enforced by this function.
     *
     * CAUTION: Does not invoke `onGRC721Received` on the receiver.
     *
     * Emits a {IGRC2309-ConsecutiveTransfer} event.
     */
    function _mintConsecutive(address to, uint96 batchSize) internal virtual returns (uint96) {
        ...
        if (batchSize > 0) {
            if (address(this).code.length > 0) {
                revert GRC721ForbiddenBatchMint();
            }
            if (to == address(0)) {
            ...
            if (batchSize > maxBatchSize) {
                revert GRC721ExceededMaxBatchMint(batchSize, maxBatchSize);
            }
    ...
    /**
     * @dev See {GRC721-_update}. Override version that restricts normal minting to after construction.
     *
     * WARNING: Using {GRC721Consecutive} prevents minting during construction in favor of {_mintConsecutive}.
