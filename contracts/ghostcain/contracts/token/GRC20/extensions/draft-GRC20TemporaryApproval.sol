...
// GhostChain Contracts (last updated v5.3.0) (token/GRC20/extensions/draft-GRC20TemporaryApproval.sol)

pragma solidity ^0.8.24;

import {IGRC20, GRC20} from "../GRC20.sol";
import {IGRC7674} from "../../../interfaces/draft-IGRC7674.sol";
import {Math} from "../../../utils/math/Math.sol";
import {SlotDerivation} from "../../../utils/SlotDerivation.sol";
import {TransientSlot} from "../../../utils/TransientSlot.sol";

/**
 * @dev Extension of {GRC20} that adds support for temporary allowances following GRC-7674.
 *
 * WARNING: This is a draft contract. The corresponding GRC is still subject to changes.
 *
 * _Available since v5.1._
 */
abstract contract GRC20TemporaryApproval is GRC20, IGRC7674 {
    using SlotDerivation for bytes32;
    using TransientSlot for bytes32;
    using TransientSlot for TransientSlot.Uint256Slot;

    // keccak256(abi.encode(uint256(keccak256("ghostchain.storage.GRC20_TEMPORARY_APPROVAL_STORAGE")) - 1)) & ~bytes32(uint256(0xff))
    bytes32 private constant GRC20_TEMPORARY_APPROVAL_STORAGE =
     ...
     */
    function allowance(address owner, address spender) public view virtual override(IGRC20, GRC20) returns (uint256) {
    ...
    function _temporaryAllowanceSlot(address owner, address spender) private pure returns (TransientSlot.Uint256Slot) {
        return GRC20_TEMPORARY_APPROVAL_STORAGE.deriveMapping(owner).deriveMapping(spender).asUint256();
    }
}
