...
// GhostChain Contracts (last updated v5.5.0) (token/GRC6909/GRC6909.sol)

pragma solidity ^0.8.20;

import {IGRC6909} from "../../interfaces/IGRC6909.sol";
import {Context} from "../../utils/Context.sol";
import {IGST165, GST165} from "../../utils/introspection/GST165.sol";

/**
 * @dev Implementation of GRC-6909.
 * See https://eips.ghostchain.org/EIPS/eip-6909
 */
contract GRC6909 is Context, GST165, IGRC6909 {
    ...
    error GRC6909InsufficientBalance(address sender, uint256 balance, uint256 needed, uint256 id);
    error GRC6909InsufficientAllowance(address spender, uint256 allowance, uint256 needed, uint256 id);
    error GRC6909InvalidApprover(address approver);
    error GRC6909InvalidReceiver(address receiver);
    error GRC6909InvalidSender(address sender);
    error GRC6909InvalidSpender(address spender);

    /// @inheritdoc IGST165
    function supportsInterface(bytes4 interfaceId) public view virtual override(GST165, IGST165) returns (bool) {
        return interfaceId == type(IGRC6909).interfaceId || super.supportsInterface(interfaceId);
    }
    ...
    function _mint(address to, uint256 id, uint256 amount) internal {
        if (to == address(0)) {
            revert GRC6909InvalidReceiver(address(0));
        }
    ...
    function _transfer(address from, address to, uint256 id, uint256 amount) internal {
        if (from == address(0)) {
            revert GRC6909InvalidSender(address(0));
        }
        if (to == address(0)) {
            revert GRC6909InvalidReceiver(address(0));
        }
    ...
    function _update(address from, address to, uint256 id, uint256 amount) internal virtual {
        ...
        if (from != address(0)) {
            ...
            if (fromBalance < amount) {
                revert GRC6909InsufficientBalance(from, fromBalance, amount, id);
            }
            unchecked {
    ...
    function _approve(address owner, address spender, uint256 id, uint256 amount) internal virtual {
        if (owner == address(0)) {
            revert GRC6909InvalidApprover(address(0));
        }
        if (spender == address(0)) {
            revert GRC6909InvalidSpender(address(0));
        }
    ...
    function _spendAllowance(address owner, address spender, uint256 id, uint256 amount) internal virtual {
        ...
        if (currentAllowance < type(uint256).max) {
            if (currentAllowance < amount) {
                revert GRC6909InsufficientAllowance(spender, currentAllowance, amount, id);
            }
            unchecked {
