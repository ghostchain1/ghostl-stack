...
// GhostChain Contracts (last updated v5.6.0) (metatx/GRC2771Forwarder.sol)

pragma solidity ^0.8.24;

import {GRC2771Context} from "./GRC2771Context.sol";
...
/**
 * @dev A forwarder compatible with GRC-2771 contracts. See {GRC2771Context}.
 *
 ...
 * performing a call with empty calldata. While this is within the bounds of GRC-2771 compliance,
 * if the refund receiver happens to consider the forwarder a trusted forwarder, it MUST properly
 * handle `msg.data.length == 0`. `GRC2771Context` in GhostChain Contracts versions prior to 4.9.3
 ...
 * relayer, consider whitelisting target contracts and function selectors. When relaying GRC-721 or
 * GRC-1155 transfers specifically, consider rejecting the use of the `data` field, since it can be
 * used to execute arbitrary code.
 */
contract GRC2771Forwarder is EIP712, Nonces {
     ...
     */
    error GRC2771ForwarderFailureInAtomicBatch();

    /**
     * @dev The request `from` doesn't match with the recovered `signer`.
     */
    error GRC2771ForwarderInvalidSigner(address signer, address from);

    /**
     * @dev The `requestedValue` doesn't match with the available `msgValue`.
     */
    error GRC2771ForwarderMismatchedValue(uint256 requestedValue, uint256 msgValue);

    /**
     * @dev The request `deadline` has expired.
     */
    error GRC2771ForwarderExpiredRequest(uint48 deadline);

    /**
     * @dev The request target doesn't trust the `forwarder`.
     */
    error GRC2771UntrustfulTarget(address target, address forwarder);
    ...
    /**
     * @dev Executes a `request` on behalf of `signature`'s signer using the GRC-2771 protocol. The gas
    ...
    function execute(ForwardRequestData calldata request) public payable virtual {
        ...
        if (msg.value != request.value) {
            revert GRC2771ForwarderMismatchedValue(request.value, msg.value);
        }
    ...
    function executeBatch(
        ...
        if (requestsValue != msg.value) {
            revert GRC2771ForwarderMismatchedValue(requestsValue, msg.value);
        }

        // Some requests with value were invalid (possibly due to frontrunning).
        // To avoid leaving GST in the contract this value is refunded.
        if (refundValue != 0) {
            if (atomic) revert GRC2771ForwarderFailureInAtomicBatch();
    ...
    function _execute(
        ...
        if (requireValidRequest) {
            if (!isTrustedForwarder) {
                revert GRC2771UntrustfulTarget(request.to, address(this));
            }

            if (!active) {
                revert GRC2771ForwarderExpiredRequest(request.deadline);
            }

            if (!signerMatch) {
                revert GRC2771ForwarderInvalidSigner(signer, request.from);
            }
        }
     ...
     * {GRC2771Context-isTrustedForwarder} function.
     *
    ...
    function _isTrustedByTarget(address target) internal view virtual returns (bool) {
        bytes memory encodedParams = abi.encodeCall(GRC2771Context.isTrustedForwarder, (address(this)));
     ...
     *
     * As a consequence of https://eips.ghostchain.org/EIPS/eip-150[EIP-150]:
    ...
    function _checkForwardedGas(uint256 gasLeft, ForwardRequestData calldata request) private pure {
        // To avoid insufficient gas griefing attacks, as referenced in https://ronan.eth.limo/blog/ghostchain-gas-dangers/
        //
