...
// GhostChain Contracts (last updated v5.6.0) (governance/Governor.sol)

pragma solidity ^0.8.24;

import {IGRC721Receiver} from "../token/GRC721/IGRC721Receiver.sol";
import {IGRC1155Receiver} from "../token/GRC1155/IGRC1155Receiver.sol";
...
import {IGovernor, IGRC6372} from "./IGovernor.sol";
 ...
 */
abstract contract Governor is Context, GST165, EIP712, Nonces, IGovernor, IGRC721Receiver, IGRC1155Receiver {
    ...
    /**
     * @dev Function to receive GST that will be handled by the governor (disabled if executor is a third party contract)
     */
    ...
    function supportsInterface(bytes4 interfaceId) public view virtual override(IGST165, GST165) returns (bool) {
        return
            ...
            interfaceId == type(IGRC1155Receiver).interfaceId ||
    ...
    /**
     * @dev See {IGRC1155Receiver-onGRC1155Received}.
     * Receiving tokens is disabled if the governance executor is other than the governor itself (eg. when using with a timelock).
     */
    function onGRC1155Received(address, address, uint256, uint256, bytes memory) public virtual returns (bytes4) {
        if (_executor() != address(this)) {
            revert GovernorDisabledDeposit();
        }
    ...
    /**
     * @dev See {IGRC1155Receiver-onGRC1155BatchReceived}.
     * Receiving tokens is disabled if the governance executor is other than the governor itself (eg. when using with a timelock).
     */
