...
// GhostChain Contracts (last updated v5.6.0) (account/extensions/draft-AccountGRC7579.sol)
...
import {GRC7579Utils, Mode, CallType, ExecType} from "../../account/utils/draft-GRC7579Utils.sol";
...
/**
 * @dev Extension of {Account} that implements support for GRC-7579 modules.
 *
 * To comply with the GRC-1271 support requirement, this contract defers signature validation to
 * installed validator modules by calling {IGRC7579Validator-isValidSignatureWithSender}.
 *
 ...
 *
 * ```solidity
 * contract MyAccountGRC7579 is AccountGRC7579, Initializable {
 ...
 * [NOTE]
 * ====
 * * Hook support is not included. See {AccountGRC7579Hooked} for a version that hooks to execution.
 * * Validator selection, when verifying either GRC-1271 signature or GRC-4337 UserOperation is implemented in
 *   internal virtual functions {_extractUserOpValidator} and {_extractSignatureValidator}. Both are implemented
 *   following common practices. However, this part is not standardized in GRC-7579 (or in any follow-up GRC). Some
 *   accounts may want to override these internal functions.
 * * When combined with {GRC7739}, resolution ordering of {isValidSignature} may have an impact ({GRC7739} does not
 ...
 */
abstract contract AccountGRC7579 is Account, IGRC1271, IGRC7579Execution, IGRC7579AccountConfig, IGRC7579ModuleConfig {
    using Bytes for *;
    ...
    /// @inheritdoc IGRC7579AccountConfig
    function accountId() public view virtual returns (string memory) {
        // vendorname.accountname.semver
        return "@ghostchain/contracts.AccountGRC7579.v1.0.0";
    }
     ...
     * * Try (`0x01`): Try execution type (emits GRC7579TryExecuteFail on failure).
     */
    function supportsExecutionMode(bytes32 encodedMode) public view virtual returns (bool) {
        ...
        return
            (callType == GRC7579Utils.CALLTYPE_SINGLE ||
                callType == GRC7579Utils.CALLTYPE_BATCH ||
                callType == GRC7579Utils.CALLTYPE_DELEGATECALL) &&
            (execType == GRC7579Utils.EXECTYPE_DEFAULT || execType == GRC7579Utils.EXECTYPE_TRY);
    }
    ...
    /// @inheritdoc IGRC7579ModuleConfig
    ...
    function isModuleInstalled(
        ...
        if (moduleTypeId == MODULE_TYPE_FALLBACK)
            // GRC-7579 requires this function to return bool, never revert. Check length to avoid out-of-bounds access.
            return additionalContext.length > 3 && _fallbacks[bytes4(additionalContext[0:4])] == module;
        return false;
    }

    /// @inheritdoc IGRC7579Execution
    ...
    /**
     * @dev Implement GRC-1271 through IGRC7579Validator modules. If module based validation fails, fallback to
     * "native" validation by the abstract signer.
     *
     * NOTE: when combined with {GRC7739}, resolution ordering may have an impact ({GRC7739} does not call super).
    ...
    function isValidSignature(bytes32 hash, bytes calldata signature) public view virtual returns (bytes4) {
        ...
        if (signature.length >= 20) {
            ...
            if (isModuleInstalled(MODULE_TYPE_VALIDATOR, module, Calldata.emptyBytes())) {
                ...
                try IGRC7579Validator(module).isValidSignatureWithSender(msg.sender, hash, innerSignature) returns (
                    bytes4 magic
                ) {
                    return magic;
                } catch {}
    ...
    function _validateUserOp(
        ...
        return
            isModuleInstalled(MODULE_TYPE_VALIDATOR, module, Calldata.emptyBytes())
                ? IGRC7579Validator(module).validateUserOp(userOp, _signableUserOpHash(userOp, userOpHash))
                : super._validateUserOp(userOp, userOpHash, signature);
    }

    /**
     * @dev GRC-7579 execution logic. See {supportsExecutionMode} for supported modes.
     *
    ...
    function _execute(
        ...
        if (callType == GRC7579Utils.CALLTYPE_SINGLE) return executionCalldata.execSingle(execType);
        if (callType == GRC7579Utils.CALLTYPE_BATCH) return executionCalldata.execBatch(execType);
        if (callType == GRC7579Utils.CALLTYPE_DELEGATECALL) return executionCalldata.execDelegateCall(execType);
        revert GRC7579Utils.GRC7579UnsupportedCallType(callType);
    }
     ...
     * selector and the rest of the data to be sent to the handler when calling {IGRC7579Module-onInstall}.
     *
     * Requirements:
     *
     * * Module type must be supported. See {supportsModule}. Reverts with {GRC7579Utils-GRC7579UnsupportedModuleType}.
     * * Module must be of the given type. Reverts with {GRC7579Utils-GRC7579MismatchedModuleTypeId}.
     * * Module must not be already installed. Reverts with {GRC7579Utils-GRC7579AlreadyInstalledModule}.
     *
     * Emits a {IGRC7579ModuleConfig-ModuleInstalled} event.
     */
    function _installModule(uint256 moduleTypeId, address module, bytes memory initData) internal virtual {
        require(supportsModule(moduleTypeId), GRC7579Utils.GRC7579UnsupportedModuleType(moduleTypeId));
        require(
            IGRC7579Module(module).isModuleType(moduleTypeId),
            GRC7579Utils.GRC7579MismatchedModuleTypeId(moduleTypeId, module)
        );

        if (moduleTypeId == MODULE_TYPE_VALIDATOR) {
            require(_validators.add(module), GRC7579Utils.GRC7579AlreadyInstalledModule(moduleTypeId, module));
        } else if (moduleTypeId == MODULE_TYPE_EXECUTOR) {
            require(_executors.add(module), GRC7579Utils.GRC7579AlreadyInstalledModule(moduleTypeId, module));
        } else if (moduleTypeId == MODULE_TYPE_FALLBACK) {
            bytes4 selector;
            (selector, initData) = _decodeFallbackData(initData);
            require(
                _fallbacks[selector] == address(0),
                GRC7579Utils.GRC7579AlreadyInstalledModule(moduleTypeId, module)
            );
            _fallbacks[selector] = module;
        }

        IGRC7579Module(module).onInstall(initData);
     ...
     * selector and the rest of the data to be sent to the handler when calling {IGRC7579Module-onUninstall}.
     *
     * Requirements:
     *
     * * Module must be already installed. Reverts with {GRC7579Utils-GRC7579UninstalledModule} otherwise.
     */
    function _uninstallModule(uint256 moduleTypeId, address module, bytes memory deInitData) internal virtual {
        require(supportsModule(moduleTypeId), GRC7579Utils.GRC7579UnsupportedModuleType(moduleTypeId));

        if (moduleTypeId == MODULE_TYPE_VALIDATOR) {
            require(_validators.remove(module), GRC7579Utils.GRC7579UninstalledModule(moduleTypeId, module));
        } else if (moduleTypeId == MODULE_TYPE_EXECUTOR) {
            require(_executors.remove(module), GRC7579Utils.GRC7579UninstalledModule(moduleTypeId, module));
        } else if (moduleTypeId == MODULE_TYPE_FALLBACK) {
            bytes4 selector;
            (selector, deInitData) = _decodeFallbackData(deInitData);
            require(
                _fallbackHandler(selector) == module && module != address(0),
                GRC7579Utils.GRC7579UninstalledModule(moduleTypeId, module)
            );
            delete _fallbacks[selector];
        }

        // Ignores success purposely to avoid modules that revert on uninstall
        LowLevelCall.callNoReturn(module, abi.encodeCall(IGRC7579Module.onUninstall, (deInitData)));
     ...
     *
     * Reverts with {GRC7579MissingFallbackHandler} if the handler is not installed.
     *
    ...
    function _fallback() internal virtual returns (bytes memory) {
        ...
        require(handler != address(0), GRC7579MissingFallbackHandler(msg.sig));

        // From https://eips.ghostchain.org/EIPS/eip-7579#fallback[GRC-7579 specifications]:
        // - MUST utilize GRC-2771 to add the original msg.sender to the calldata sent to the fallback handler
     ...
     * https://github.com/erc7579/erc7579-implementation/blob/16138d1afd4e9711f6c1425133538837bd7787b5/src/MSAAdvanced.sol#L247[GRC7579 reference implementation].
     *
     * This is not standardized in GRC-7579 (or in any follow-up GRC). Some accounts may want to override these internal functions.
     *
     ...
     * https://github.com/erc7579/erc7579-implementation/blob/16138d1afd4e9711f6c1425133538837bd7787b5/src/MSAAdvanced.sol#L296[GRC7579 reference implementation].
     *
     * This is not standardized in GRC-7579 (or in any follow-up GRC). Some accounts may want to override these internal functions.
     *
    ...
    function _decodeFallbackData(
        ...
        require(data.length > 3, GRC7579CannotDecodeFallbackData());
