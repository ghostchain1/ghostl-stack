...
// GhostChain Contracts (last updated v5.6.0) (account/utils/draft-GRC7579Utils.sol)

pragma solidity ^0.8.20;

import {Execution} from "../../interfaces/draft-IGRC7579.sol";
...
/**
 * @dev Library with common GRC-7579 utility functions.
 *
 * See https://eips.ghostchain.org/EIPS/eip-7579[GRC-7579].
 */
// slither-disable-next-line unused-state
library GRC7579Utils {
    using Packing for *;
     ...
     */
    event GRC7579TryExecuteFail(uint256 batchExecutionIndex, bytes returndata);

    /// @dev The provided {CallType} is not supported.
    error GRC7579UnsupportedCallType(CallType callType);

    /// @dev The provided {ExecType} is not supported.
    error GRC7579UnsupportedExecType(ExecType execType);

    /// @dev The provided module doesn't match the provided module type.
    error GRC7579MismatchedModuleTypeId(uint256 moduleTypeId, address module);

    /// @dev The module is not installed.
    error GRC7579UninstalledModule(uint256 moduleTypeId, address module);

    /// @dev The module is already installed.
    error GRC7579AlreadyInstalledModule(uint256 moduleTypeId, address module);

    /// @dev The module type is not supported.
    error GRC7579UnsupportedModuleType(uint256 moduleTypeId);

    /// @dev Input calldata not properly formatted and possibly malicious.
    error GRC7579DecodingError();
    ...
    ///
    /// NOTE: This function runs some checks and will throw a {GRC7579DecodingError} if the input is not properly formatted.
    function decodeBatch(bytes calldata executionCalldata) internal pure returns (Execution[] calldata executionBatch) {
        unchecked {
            uint256 bufferLength = executionCalldata.length;

            // Check executionCalldata is not empty.
            if (bufferLength < 0x20) revert GRC7579DecodingError();
            ...
            if (arrayLengthOffset > bufferLength - 0x20) revert GRC7579DecodingError();
    ...
    function _validateExecutionMode(
        ...
        if (execType == GRC7579Utils.EXECTYPE_DEFAULT) {
            Address.verifyCallResult(success, returndata);
        } else if (execType == GRC7579Utils.EXECTYPE_TRY) {
            if (!success) emit GRC7579TryExecuteFail(index, returndata);
        } else {
            revert GRC7579UnsupportedExecType(execType);
        }
        return returndata;
    }
}
