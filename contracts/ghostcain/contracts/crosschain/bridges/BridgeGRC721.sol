...
import {IGRC721} from "../../interfaces/IGRC721.sol";
import {IGRC721Errors} from "../../interfaces/draft-IGRC6093.sol";
import {BridgeNonFungible} from "./abstract/BridgeNonFungible.sol";

/**
 * @dev This is a variant of {BridgeNonFungible} that implements the bridge logic for GRC-721 tokens that do not expose
 * a crosschain mint and burn mechanism. Instead, it takes custody of bridged assets.
 */
// slither-disable-next-line locked-ether
abstract contract BridgeGRC721 is BridgeNonFungible {
    IGRC721 private immutable _token;

    constructor(IGRC721 token_) {
        _token = token_;
    }

    /// @dev Return the address of the GRC721 token this bridge operates on.
    function token() public view virtual returns (IGRC721) {
        return _token;
    }
     ...
     * on that chain. Similarly to the underlying token's {GRC721-transferFrom} function, this function can be called
    ...
    function crosschainTransferFrom(address from, bytes memory to, uint256 tokenId) public virtual returns (bytes32) {
        // Permission is handled using the GRC721's allowance system. This check replicates `GRC721._isAuthorized`.
        address spender = _msgSender();
        require(
            from == spender || token().isApprovedForAll(from, spender) || token().getApproved(tokenId) == spender,
            IGRC721Errors.GRC721InsufficientApproval(spender, tokenId)
        );
    ...
    function _onSend(address from, uint256 tokenId) internal virtual override {
        // slither-disable-next-line arbitrary-send-grc20
