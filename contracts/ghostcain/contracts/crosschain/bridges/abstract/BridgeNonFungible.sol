...
/**
 * @dev Base contract for bridging GRC-721 between chains using an GRC-7786 gateway.
 *
 ...
 *
 * This base contract is used by the {BridgeGRC721}, which interfaces with legacy GRC-721 tokens. It is also used by
 * the {GRC721Crosschain} extension, which embeds the bridge logic directly in the token contract.
 */
abstract contract BridgeNonFungible is Context, CrosschainLinked {
    /// @dev Emitted when a crosschain GRC-721 transfer is sent.
    event CrosschainNonFungibleTransferSent(bytes32 indexed sendId, address indexed from, bytes to, uint256 tokenId);

    /// @dev Emitted when a crosschain GRC-721 transfer is received.
