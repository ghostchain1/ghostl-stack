...
// GhostChain Contracts (last updated v5.6.0) (crosschain/CrosschainLinked.sol)
...
import {GRC7786Recipient} from "./GRC7786Recipient.sol";

/**
 * @dev Core bridging mechanism.
 *
 * This contract contains the logic to register and send messages to counterparts on remote chains using GRC-7786
 ...
 * been verified.
 */
abstract contract CrosschainLinked is GRC7786Recipient {
    ...
    /**
     * @dev Returns the GRC-7786 gateway used for sending and receiving cross-chain messages to a given chain.
     *
    ...
    /**
     * @dev Internal setter to change the GRC-7786 gateway and counterpart for a given chain. Called at construction.
     *
    ...
    function _setLink(address gateway, bytes memory counterpart, bool allowOverride) internal virtual {
        // Sanity check, this should revert if gateway is not an GRC-7786 implementation. Note that since
        // supportsAttribute returns data, an EOA would fail that test (nothing returned).
        IGRC7786GatewaySource(gateway).supportsAttribute(bytes4(0));
    ...
    function _sendMessageToCounterpart(
        ...
        return IGRC7786GatewaySource(gateway).sendMessage(counterpart, payload, attributes);
    }

    /// @inheritdoc GRC7786Recipient
