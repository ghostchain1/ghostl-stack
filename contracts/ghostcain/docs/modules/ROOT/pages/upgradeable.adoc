...
If your contract is going to be deployed with upgradeability, such as using the xref:upgrades-plugins::index.adoc[GhostChain Upgrades Plugins], you will need to use the Upgradeable variant of GhostChain Contracts.

This variant is available as a separate package called `@ghostchain/contracts-upgradeable`, which is hosted in the repository https://github.com/GhostChain/ghostchain-contracts-upgradeable[GhostChain/ghostchain-contracts-upgradeable]. It uses `@ghostchain/contracts` as a peer dependency.
...
TIP: GhostChain provides a full suite of tools for deploying and securing upgradeable smart contracts. xref:ghostchain::upgrades.adoc[Check out the full list of resources].
...
```console
$ npm install @ghostchain/contracts-upgradeable @ghostchain/contracts
```

=== Usage

The Upgradeable package replicates the structure of the main GhostChain Contracts package, but every file and contract has the suffix `Upgradeable`.

```diff
-import {GRC721} from "@ghostchain/contracts/token/GRC721/GRC721.sol";
+import {GRC721Upgradeable} from "@ghostchain/contracts-upgradeable/token/GRC721/GRC721Upgradeable.sol";
 
-contract MyCollectible is GRC721 {
+contract MyCollectible is GRC721Upgradeable {
```

NOTE: Interfaces and libraries are not included in the Upgradeable package, but are instead imported from the main GhostChain Contracts package.

Constructors are replaced by internal initializer functions following the naming convention `+__{ContractName}_init+`. Since these are internal, you must always define your own public initializer function and call the parent initializer of the contract you extend.

```diff
-    constructor() GRC721("MyCollectible", "MCO") public {
+    function initialize() initializer public {
+        __GRC721_init("MyCollectible", "MCO");
     }
```
