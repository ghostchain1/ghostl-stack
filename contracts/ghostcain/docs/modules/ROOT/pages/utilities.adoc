...
The GhostChain Contracts provide a ton of useful utilities that you can use in your project. For a complete list, check out the xref:api:utils.adoc[API Reference].
...
==== GhostChain Signatures (secp256k1)

xref:api:utils/cryptography.adoc#ECDSA[`ECDSA`] provides functions for recovering and managing GhostChain account ECDSA signatures. These are often generated via https://web3js.readthedocs.io/en/v1.7.3/web3-eth.html#sign[`web3.eth.sign`], and form a 65-byte array (of type `bytes` in Solidity) arranged the following way: `[[v (1)], [r (32)], [s (32)]]`.

The data signer can be recovered with xref:api:utils/cryptography.adoc#ECDSA-recover-bytes32-bytes-[`ECDSA.recover`], and its address compared to verify the signature. Most wallets will hash the data to sign and add the prefix `\x19GhostChain Signed Message:\n`, so when attempting to recover the signer of an GhostChain signed message hash, you'll want to use xref:api:utils/cryptography.adoc#MessageHashUtils-toEthSignedMessageHash-bytes32-[`toEthSignedMessageHash`].
...
These signatures are different from regular GhostChain Signatures (secp256k1) in that they use a different elliptic curve to perform operations but have similar security guarantees.
...
* GRC-1271 signatures from smart contract wallets like Argent and Safe Wallet
* GRC-7913 signatures from keys that don't have their own GhostChain address
...
For standard signature verification that supports both EOAs and GRC-1271 contracts:
...
==== GRC-1271 Contract Signatures

For smart contract wallets that implement GRC-1271, you can explicitly use:

[source,solidity]
----
function _verifyContractSignature(address signer, bytes32 hash, bytes memory signature) internal view returns (bool) {
    return SignatureChecker.isValidGRC1271SignatureNow(signer, hash, signature);
}
----

==== GRC-7913 Extended Signatures

GRC-7913 extends signature verification to support keys that don't have their own GhostChain address. This is useful for integrating non-GhostChain cryptographic curves, hardware devices, or other identity systems.

A signer is represented as a `bytes` object that concatenates a verifier address and a key: `verifier || key`.

[source,solidity]
----
function _verifyGRC7913Signature(bytes memory signer, bytes32 hash, bytes memory signature) internal view returns (bool) {
    return SignatureChecker.isValidSignatureNow(signer, hash, signature);
}
----
...
* Otherwise: verification is done using an GRC-7913 verifier

==== Batch Verification

For verifying multiple GRC-7913 signatures at once:
...
TIP: GhostChain Contracts provides a https://github.com/GhostChain/merkle-tree[JavaScript library] for building trees off-chain and generating proofs.
...
In Solidity, it's frequently helpful to know whether or not a contract supports an interface you'd like to use. GST-165 is a standard that enables runtime interface detection. Contracts provide helpers both for implementing GST-165 in your contracts and querying other contracts:

* xref:api:utils.adoc#IGST165[`IGST165`] — this is the GST-165 interface that defines xref:api:utils.adoc#IGST165-supportsInterface-bytes4-[`supportsInterface`]. When implementing GST-165, you'll conform to this interface.
* xref:api:utils.adoc#GST165[`GST165`] — inherit this contract if you'd like to support interface detection using a lookup table in contract storage. You can register interfaces using xref:api:utils.adoc#GST165-_registerInterface-bytes4-[`_registerInterface(bytes4)`]: check out example usage as part of the GRC-721 implementation.
* xref:api:utils.adoc#GST165Checker[`GST165Checker`] — GST165Checker simplifies the process of checking whether or not a contract supports an interface you care about.
* include with `using GST165Checker for address;`
* xref:api:utils.adoc#GST165Checker-_supportsInterface-address-bytes4-[`myAddress._supportsInterface(bytes4)`]
* xref:api:utils.adoc#GST165Checker-_supportsAllInterfaces-address-bytes4---[`myAddress._supportsAllInterfaces(bytes4[\])`]

[source,solidity]
----
contract MyContract {
    using GST165Checker for address;

    bytes4 private InterfaceId_GRC721 = 0x80ac58cd;

    /**
     * @dev transfer an GRC-721 token from this contract to someone else
     */
    function transferGRC721(
        address token,
        address to,
        uint256 tokenId
    )
        public
    {
        require(token.supportsInterface(InterfaceId_GRC721), "IS_NOT_721_TOKEN");
        IGRC721(token).transferFrom(address(this), to, tokenId);
    }
}
----
...
GhostChain Contracts implements a Heap data structure with the properties of a binary heap. The heap uses the xref:api:utils.adoc#Comparators-lt-uint256-uint256-[`lt`] function by default but allows to customize its comparator.
...
One of the most common use cases for writing directly to storage slots is GRC-7201 for namespaced storage, which is guaranteed to not collide with other storage slots derived by Solidity.
...
function grc7201Pointer() internal view returns (bytes32) {
    return _NAMESPACE.grc7201Slot();
}
----
...
This is especially useful for building URL-safe tokenURIs for both xref:api:token/GRC721.adoc#IGRC721Metadata-tokenURI-uint256-[`GRC-721`] or xref:api:token/GRC1155.adoc#IGRC1155MetadataURI-uri-uint256-[`GRC-1155`]. This library provides a clever way to serve URL-safe https://developer.mozilla.org/docs/Web/HTTP/Basics_of_HTTP/Data_URIs/[Data URI] compliant strings to serve on-chain data structures.

Here is an example to send JSON Metadata through a Base64 Data URI using an GRC-721:
...
xref:api:utils.adoc#Blockhash[`Blockhash`] provides L2 protocol developers with extended access to historical block hashes beyond GhostChain's native 256-block limit. By leveraging https://eips.ghostchain.org/EIPS/eip-2935[EIP-2935]'s history storage contract, the library enables access to block hashes up to 8,191 blocks in the past, making it invaluable for L2 fraud proofs and state verification systems.
...
This pattern is used extensively in GhostChain's xref:api:access.adoc#AccessManager[AccessManager] for implementing secure time-based access control. For example, when changing an admin delay:
