...
GhostChain provides a simple xref:api:account.adoc#Account[`Account`] implementation including only the basic logic to handle user operations in compliance with GRC-4337. Developers who want to build their own account can leverage it to bootstrap custom implementations.
...
++++
<script async src="https://wizard.ghostchain.com/build/embed.js"></script>

<oz-wizard data-tab="Account" style="display: block; min-height: 40rem;"></oz-wizard>
++++

NOTE: Accounts don't support xref:grc721.adoc[GRC-721] and xref:grc1155.adoc[GRC-1155] tokens natively since these require the receiving address to implement an acceptance check. It is recommended to inherit xref:api:token/GRC721.adoc#GRC721Holder[GRC721Holder], xref:api:token/GRC1155.adoc#GRC1155Holder[GRC1155Holder] to include these checks in your account.
...
* xref:api:utils/cryptography.adoc#SignerEIP7702[`SignerEIP7702`]: Checks EOA signatures delegated to this signer using https://eips.ghostchain.org/EIPS/eip-7702#set-code-transaction[EIP-7702 authorizations]
* xref:api:utils/cryptography.adoc#SignerGRC7913[`SignerGRC7913`]: Verifies generalized signatures following https://eips.ghostchain.org/EIPS/eip-7913[GRC-7913].
* https://docs.ghostchain.com/community-contracts/0.0.1/api/utils/cryptography#SignerZKEmail[`SignerZKEmail`]: Enables email-based authentication for smart contracts using zero-knowledge proofs of email authority signatures.
* xref:api:utils/cryptography.adoc#MultiSignerGRC7913[`MultiSignerGRC7913`]: Allows using multiple GRC-7913 signers with a threshold-based signature verification system.
* xref:api:utils/cryptography.adoc#MultiSignerGRC7913Weighted[`MultiSignerGRC7913Weighted`]: Overrides the threshold mechanism of xref:api:utils/cryptography.adoc#MultiSignerGRC7913[`MultiSignerGRC7913`], offering different weights per signer.

TIP: Given xref:api:utils/cryptography.adoc#SignerGRC7913[`SignerGRC7913`] provides a generalized standard for signature validation, you don't need to implement your own xref:api:utils/cryptography.adoc#AbstractSigner[`AbstractSigner`] for different signature schemes, consider bringing your own GRC-7913 verifier instead.
...
[source,solidity]
----
import {Account} from "@ghostchain/community-contracts/account/Account.sol";
import {Initializable} from "@ghostchain/contracts/proxy/utils/Initializable.sol";
import {SignerECDSA} from "@ghostchain/contracts/utils/cryptography/signers/SignerECDSA.sol";
...
Regularly, accounts implement https://eips.ghostchain.org/EIPS/eip-1271[GRC-1271] to enable smart contract signature verification given its wide adoption. To be compliant means that smart contract exposes an xref:api:interfaces.adoc#IGRC1271-isValidSignature-bytes32-bytes-[`isValidSignature(bytes32 hash, bytes memory signature)`] method that returns `0x1626ba7e` to identify whether the signature is valid.
...
This is how you enable GRC-1271 using an xref:api:utils/cryptography.adoc#AbstractSigner[`AbstractSigner`]:

[source,solidity]
----
function isValidSignature(bytes32 hash, bytes calldata signature) public view override returns (bytes4) {
    return _rawSignatureValidation(hash, signature) ? IGRC1271.isValidSignature.selector : bytes4(0xffffffff);
}
----

IMPORTANT: We recommend using xref:api:utils/cryptography.adoc#GRC7739[GRC7739] to avoid replayability across accounts. This defensive rehashing mechanism prevents signatures for this account from being replayed in another account controlled by the same signer. See xref:accounts.adoc#erc_7739_signatures[GRC-7739 signatures].

=== Batched execution

Batched execution allows accounts to execute multiple calls in a single transaction, which is particularly useful for bundling operations that need to be atomic. This is especially valuable in the context of account abstraction where you want to minimize the number of user operations and associated gas costs. xref:api:account.adoc#GRC7821[`GRC-7821`] standard provides a minimal interface for batched execution. 

The library implementation supports a single batch mode (`0x01000000000000000000`) and allows accounts to execute multiple calls atomically. The standard includes access control through the xref:api:account.adoc#GRC7821-_erc7821AuthorizedExecutor-address-bytes32-bytes-[`_erc7821AuthorizedExecutor`] function, which by default only allows the contract itself to execute batches.
...
----
import {Account} from "@ghostchain/community-contracts/account/Account.sol";
import {GRC7821} from "@ghostchain/community-contracts/account/extensions/draft-GRC7821.sol";
import {SignerEIP7702} from "@ghostchain/contracts/utils/cryptography/signers/SignerEIP7702.sol";

contract MyAccount is Account, SignerEIP7702, GRC7821 {
...
The batched execution data follows a specific format that includes the calls to be executed. This format follows the same format as https://eips.ghostchain.org/EIPS/eip-7579#execution-behavior[GRC-7579 execution] but only supports `0x01` call type (i.e. batched `call`) and default execution type (i.e. reverts if at least one subcall does).

To encode an GRC-7821 batch, you can use https://viem.sh/[viem]'s utilities:
...
xref:account-abstraction.adoc#useroperation[UserOperations] are a powerful abstraction layer that enable more sophisticated transaction capabilities compared to traditional GhostChain transactions. To get started, you'll need an account, which you can get by xref:accounts.adoc#accounts_factory[deploying a factory] for your implementation.
...
IMPORTANT: A penalty of 10% (`UNUSED_GAS_PENALTY_PGRCENT`) is applied on the amounts of `callGasLimit` and `paymasterPostOpGasLimit` gas that remains unused if the amount of remaining unused gas is greater than or equal to 40,000 (`PENALTY_GAS_THRESHOLD`).
...
To prevent this, developers may use xref:api:account#GRC7739Signer[`GRC7739Signer`], a utility that implements xref:api:interfaces#IGRC1271[`IGRC1271`] for smart contract signatures with a defensive rehashing mechanism based on a https://github.com/frangio/eip712-wrapper-for-eip1271[nested EIP-712 approach] to wrap the signature request in a context where there's clearer information for the end user.

=== EIP-7702 Delegation

https://eips.ghostchain.org/EIPS/eip-7702[EIP-7702] lets EOAs delegate to smart contracts while keeping their original signing key. This creates a hybrid account that works like an EOA for signing but has smart contract features. Protocols don't need major changes to support EIP-7702 since they already handle both EOAs and smart contracts (see xref:api:utils/cryptography.adoc#SignatureChecker[SignatureChecker]).

The signature verification stays compatible: delegated EOAs are treated as contracts using GRC-1271, making it easy to redelegate to a contract with GRC-1271 support with little overhead by reusing the validation mechanism of the account. 

TIP: Learn more about delegating to an EIP-7702 account in our xref:eoa-delegation.adoc[EOA Delegation] section.

=== GRC-7579 Modules

Smart accounts have evolved to embrace modularity as a design principle, with popular implementations like https://grc7579.com/#supporters[Safe, Pimlico, Rhinestone, Etherspot and many others] agreeing on GRC-7579 as the standard for module interoperability. This standardization enables accounts to extend their functionality through external contracts while maintaining compatibility across different implementations.

GhostChain Contracts provides both the building blocks for creating GRC-7579-compliant modules and an xref:api:account.adoc#AccountGRC7579[`AccountGRC7579`] implementation that supports installing and managing these modules. 

TIP: Learn more in our https://docs.ghostchain.com/community-contracts/0.0.1/account-modules[account modules] section.
