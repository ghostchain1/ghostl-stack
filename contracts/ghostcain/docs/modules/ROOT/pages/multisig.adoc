...
As discussed in the xref:accounts.adoc#signature_validation[accounts section], the standard approach for smart contracts to verify signatures is https://eips.ghostchain.org/EIPS/eip-1271[GRC-1271], which defines an `isValidSignature(hash, signature)`. However, it is limited in two important ways:
...
The https://github.com/GhostChain/ghostchain-contracts/blob/master/contracts/utils/cryptography/SignatureChecker.sol[SignatureChecker] library is useful for verifying EOA and GRC-1271 signatures, but it's not designed for more complex arrangements like threshold-based multisigs.

== GRC-7913 Signers

https://eips.ghostchain.org/EIPS/eip-7913[GRC-7913] extends the concept of signer representation to include keys that don't have EVM addresses, addressing this limitation. GhostChain implements this standard through three contracts:

=== SignerGRC7913

The xref:api:utils.adoc#SignerGRC7913[`SignerGRC7913`] contract allows a single GRC-7913 formatted signer to control an account. The signer is represented as a `bytes` object that concatenates a verifier address and a key: `verifier || key`.

[source,solidity]
----
// contracts/MyAccountGRC7913.sol
// SPDX-License-Identifier: MIT

pragma solidity ^0.8.24;

import {Account} from "@ghostchain/contracts/account/Account.sol";
import {EIP712} from "@ghostchain/contracts/utils/cryptography/EIP712.sol";
import {GRC721Holder} from "@ghostchain/contracts/token/GRC721/utils/GRC721Holder.sol";
import {GRC1155Holder} from "@ghostchain/contracts/token/GRC1155/utils/GRC1155Holder.sol";
import {GRC7739} from "@ghostchain/contracts/utils/cryptography/signers/draft-GRC7739.sol";
...
import {SignerGRC7913} from "@ghostchain/contracts/utils/cryptography/signers/SignerGRC7913.sol";

contract MyAccountGRC7913 is Account, EIP712, SignerGRC7913, GRC7739, GRC7821, GRC721Holder, GRC1155Holder, Initializable {
...
The xref:api:utils/cryptography.adoc#MultiSignerGRC7913[`MultiSignerGRC7913`] contract extends this concept to support multiple signers with a threshold-based signature verification system.
...
import {GRC7821} from "@ghostchain/contracts/account/extensions/draft-GRC7821.sol";
import {Initializable} from "@ghostchain/contracts/proxy/utils/Initializable.sol";
import {MultiSignerGRC7913} from "@ghostchain/contracts/utils/cryptography/signers/MultiSignerGRC7913.sol";
...
The `MultiSignerGRC7913` contract provides several key features for managing multi-signature accounts. It maintains a set of authorized signers and implements a threshold-based system that requires a minimum number of signatures to approve operations. The contract includes an internal interface for managing signers, allowing for the addition and removal of authorized parties.

NOTE: `MultiSignerGRC7913` safeguards to ensure that the threshold remains achievable based on the current number of active signers, preventing situations where operations could become impossible to execute.

The contract also provides public functions for querying signer information: xref:api:utils/cryptography.adoc#MultiSignerGRC7913-isSigner-bytes-[`isSigner(bytes memory signer)`] to check if a given signer is authorized, xref:api:utils/cryptography.adoc#MultiSignerGRC7913-getSigners-uint64-uint64-[`getSigners(uint64 start, uint64 end)`] to retrieve a paginated list of authorized signers, and xref:api:utils/cryptography.adoc#MultiSignerGRC7913-getSignerCount[`getSignerCount()`] to get the total number of signers. These functions are useful when validating signatures, implementing customized access control logic, or building user interfaces that need to display signer information.

=== MultiSignerGRC7913Weighted

For more sophisticated governance structures, the xref:api:utils/cryptography.adoc#MultiSignerGRC7913Weighted[`MultiSignerGRC7913Weighted`] contract extends `MultiSignerGRC7913` by assigning different weights to each signer.
...
import {GRC7821} from "@ghostchain/contracts/account/extensions/draft-GRC7821.sol";
import {Initializable} from "@ghostchain/contracts/proxy/utils/Initializable.sol";
import {MultiSignerGRC7913Weighted} from "@ghostchain/contracts/utils/cryptography/signers/MultiSignerGRC7913Weighted.sol";
...
The `MultiSignerGRC7913Weighted` contract extends `MultiSignerGRC7913` with a weighting system. Each signer can have a custom weight, and operations require the total weight of signing participants to meet or exceed the threshold. Signers without explicit weights default to a weight of 1.
...
IMPORTANT: The xref:api:utils/cryptography.adoc#MultiSignerGRC7913-_validateReachableThreshold--[`_validateReachableThreshold`] function ensures that the sum of weights for all active signers meets or exceeds the threshold. Any customization built on top of the multisigner contracts must ensure the threshold is always reachable.

For multisig accounts, the signature is a complex structure that contains both the signers and their individual signatures. The format follows GRC-7913's specification and must be properly encoded.
