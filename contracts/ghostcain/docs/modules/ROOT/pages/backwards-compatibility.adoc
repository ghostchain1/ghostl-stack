...
=== Draft or Pre-Final GRCs

GRCs that are not Final can change in incompatible ways. For this reason, we avoid shipping implementations of GRCs before they are Final. Some exceptions are made for GRCs that have been published for a long time and seem unlikely to change. Implementations for GRCs that may have breaking changes are published in files named `draft-*.sol` to make that condition explicit. There is no backwards compatibility guarantee for content in files prefixed with `draft`.
...
We recommend using xref:upgrades-plugins::index.adoc[GhostChain Upgrades Plugins or CLI] to ensure storage layout safety of upgrades.
