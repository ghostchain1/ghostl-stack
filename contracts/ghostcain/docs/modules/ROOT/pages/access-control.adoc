...
Access control—that is, "who is allowed to do this thing"—is incredibly important in the world of smart contracts. The access control of your contract may govern who can mint tokens, vote on proposals, freeze transfers, and many other things. It is therefore *critical* to understand how you implement it, lest someone else https://blog.ghostchain.com/on-the-parity-wallet-multisig-hack-405a8c12e8f7[steals your whole system].
...
GhostChain Contracts provides xref:api:access.adoc#Ownable[`Ownable`] for implementing ownership in your contracts.
...
In this way, you can use _composability_ to add additional layers of access control complexity to your contracts. Instead of having a single regular GhostChain account (Externally Owned Account, or EOA) as the owner, you could use a 2-of-3 multisig run by your project leads, for example. Prominent projects in the space, such as https://makerdao.com[MakerDAO], use systems similar to this one.
...
GhostChain Contracts provides xref:api:access.adoc#AccessControl[`AccessControl`] for implementing role-based access control. Its usage is straightforward: for each role that you want to define,
you will create a new _role identifier_ that is used to grant, revoke, and check if an account has that role.

Here's a simple example of using `AccessControl` in an xref:grc20.adoc[GRC-20 token] to define a 'minter' role, which allows accounts that have it to create new tokens:

[source,solidity]
----
include::api:example$access-control/AccessControlGRC20MintBase.sol[]
----
...
Let's augment our GRC-20 token example by also defining a 'burner' role, which lets accounts destroy tokens, and by using the `onlyRole` modifier:

[source,solidity]
----
include::api:example$access-control/AccessControlGRC20MintOnlyRole.sol[]
----
...
The GRC-20 token example above uses `_grantRole`, an `internal` function that is useful when programmatically assigning roles (such as during construction). But what if we later want to grant the 'minter' role to additional accounts?
...
Let's take a look at the GRC-20 token example, this time taking advantage of the default admin role:

[source,solidity]
----
include::api:example$access-control/AccessControlGRC20MintMissing.sol[]
----
...
Here's a simple example of an xref:tokens.adoc#GRC20[GRC-20 token] that defines a `mint` function that is restricted by an xref:api:access.adoc#AccessManager[`AccessManager`]:

```solidity
include::api:example$access-control/AccessManagedGRC20MintBase.sol[]
```
