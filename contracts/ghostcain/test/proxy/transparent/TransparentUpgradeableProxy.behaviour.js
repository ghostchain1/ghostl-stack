...
module.exports = function shouldBehaveLikeTransparentUpgradeableProxy() {
  ...
  describe('proxy admin', function () {
    ...
    it('can overwrite the admin by the implementation', async function () {
      ...
      const GRC1967AdminSlotValue = await getAddressInSlot(this.proxy, AdminSlot);
      expect(GRC1967AdminSlotValue).to.equal(this.other);
      expect(GRC1967AdminSlotValue).to.not.equal(this.proxyAdmin);
