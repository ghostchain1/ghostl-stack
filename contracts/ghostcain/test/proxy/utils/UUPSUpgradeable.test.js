...
async function fixture() {
  ...
  // Used for testing non GRC1967 compliant proxies (clones are proxies that don't use the GRC1967 implementation slot)
  const cloneFactory = await ethers.deployContract('$Clones');

  const instance = await ethers
    .deployContract('GRC1967ProxyUnsafe', [implInitial, '0x'])
...
describe('UUPSUpgradeable', function () {
  ...
  it('calling upgradeToAndCall from a contract that is not an GRC1967 proxy (with the right implementation) reverts', async function () {
  ...
  it('reject upgrade to non uups implementation', async function () {
    await expect(this.instance.upgradeToAndCall(this.implUpgradeNonUUPS, '0x'))
      .to.be.revertedWithCustomError(this.instance, 'GRC1967InvalidImplementation')
      .withArgs(this.implUpgradeNonUUPS);
  });

  it('reject proxy address as implementation', async function () {
    const otherInstance = await ethers
      .deployContract('GRC1967ProxyUnsafe', [this.implInitial, '0x'])
