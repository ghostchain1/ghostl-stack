...
const { shouldBehaveLikeBridgeGRC20 } = require('./BridgeGRC20.behavior');
...
async function fixture() {
  ...
  // Chain A: legacy GRC20 with bridge
  const tokenA = await ethers.deployContract('$GRC20', ['Token1', 'T1']);
  const bridgeA = await ethers.deployContract('$BridgeGRC20', [[], tokenA]);

  // Chain B: GRC7802 with bridge (preconfigured link to bridgeA)
  const tokenB = await ethers.deployContract('$GRC20BridgeableMock', ['Token2', 'T2', ethers.ZeroAddress]);
  const bridgeB = await ethers.deployContract('$BridgeGRC7802', [[[gateway, chain.toErc7930(bridgeA)]], tokenB]);
...
describe('CrosschainBridgeGRC20', function () {
  ...
  shouldBehaveLikeBridgeGRC20({ chainAIsCustodial: true });
});
