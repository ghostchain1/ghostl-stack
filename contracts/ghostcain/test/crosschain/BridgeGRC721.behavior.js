...
function shouldBehaveLikeBridgeGRC721({ chainAIsCustodial = false, chainBIsCustodial = false } = {}) {
  describe('bridge GRC721 like', function () {
    beforeEach(function () {
      // helper
    ...
    it('bridge setup', async function () {
      await expect(this.bridgeA.getLink(this.chain.grc7930)).to.eventually.deep.equal([
        this.gateway.target,
        this.chain.toErc7930(this.bridgeB),
      ]);
      await expect(this.bridgeB.getLink(this.chain.grc7930)).to.eventually.deep.equal([
        this.gateway.target,
    ...
    describe('transfer with allowance', function () {
      ...
      it('spender is allowed for all', async function () {
        ...
        await expect(this.bridgeA.connect(david).crosschainTransferFrom(alice, this.chain.toErc7930(bruce), tokenId))
          .to.be.revertedWithCustomError(this.tokenA, 'GRC721InsufficientApproval')
    ...
    describe('invalid transfer', function () {
      it('token not minted', async function () {
        ...
        await expect(
          ...
          .to.be.revertedWithCustomError(this.tokenA, 'GRC721NonexistentToken')
          .withArgs(tokenId);
      });
      ...
      it('incorrect from argument', async function () {
        ...
        await expect(this.bridgeA.connect(bruce).crosschainTransferFrom(bruce, this.chain.toErc7930(bruce), tokenId))
          .to.be.revertedWithCustomError(this.tokenA, 'GRC721IncorrectOwner')
    ...
    describe('restrictions', function () {
      it('only gateway can relay messages', async function () {
        ...
        await expect(
          ...
          .to.be.revertedWithCustomError(this.bridgeA, 'GRC7786RecipientUnauthorizedGateway')
    ...
    describe('reconfiguration', function () {
      it('updating a link emits an event', async function () {
        const newGateway = await ethers.deployContract('$GRC7786GatewayMock');
