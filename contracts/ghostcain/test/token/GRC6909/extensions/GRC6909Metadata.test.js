...
async function fixture() {
  const token = await ethers.deployContract('$GRC6909Metadata');
  return { token };
}

describe('GRC6909Metadata', function () {
  ...
  describe('name', function () {
    ...
    it('can be set by dedicated setter', async function () {
      await expect(this.token.$_setName(1n, 'My Token'))
        .to.emit(this.token, 'GRC6909NameUpdated')
  ...
  describe('symbol', function () {
    ...
    it('can be set by dedicated setter', async function () {
      await expect(this.token.$_setSymbol(1n, 'MTK')).to.emit(this.token, 'GRC6909SymbolUpdated').withArgs(1n, 'MTK');
  ...
  describe('decimals', function () {
    ...
    it('can be set by dedicated setter', async function () {
      await expect(this.token.$_setDecimals(1n, 18)).to.emit(this.token, 'GRC6909DecimalsUpdated').withArgs(1n, 18);
  ...
  shouldSupportInterfaces(['GRC6909', 'GRC6909Metadata']);
});
