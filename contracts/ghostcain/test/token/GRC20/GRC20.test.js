...
} = require('./GRC20.behavior');

const TOKENS = [{ Token: '$GRC20' }, { Token: '$GRC20ApprovalMock', forcedApproval: true }];

const name = 'My Token';
const symbol = 'MTKN';
const initialSupply = 100n;

describe('GRC20', function () {
  for (const { Token, forcedApproval } of TOKENS) {
    describe(Token, function () {
      ...
      shouldBehaveLikeGRC20(initialSupply, { forcedApproval });
      ...
      describe('_burn', function () {
        it('rejects a null account', async function () {
          await expect(this.token.$_burn(ethers.ZeroAddress, 1n))
            .to.be.revertedWithCustomError(this.token, 'GRC20InvalidSender')
            .withArgs(ethers.ZeroAddress);
        });
      ...
      describe('_approve', function () {
        ...
        it('reverts when the owner is the zero address', async function () {
          await expect(this.token.$_approve(ethers.ZeroAddress, this.recipient, initialSupply))
            .to.be.revertedWithCustomError(this.token, 'GRC20InvalidApprover')
            .withArgs(ethers.ZeroAddress);
        });
      });
    });
