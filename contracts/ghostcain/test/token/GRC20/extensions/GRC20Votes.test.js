...
describe('GRC20Votes', function () {
  for (const { Token, mode } of TOKENS) {
    ...
    describe(`vote with ${mode}`, function () {
      ...
      // includes GRC6372 behavior check
      ...
      it('minting restriction', async function () {
        ...
        await expect(this.token.$_mint(this.holder, value))
          .to.be.revertedWithCustomError(this.token, 'GRC20ExceededSafeSupply')
        ...
        describe('getPastVotes', function () {
          it('reverts if block number >= current block', async function () {
            ...
            await expect(this.token.getPastVotes(this.other1, 50_000_000_000n))
              .to.be.revertedWithCustomError(this.token, 'GRC5805FutureLookup')
