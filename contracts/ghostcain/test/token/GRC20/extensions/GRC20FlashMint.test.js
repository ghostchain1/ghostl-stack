...
async function fixture() {
  ...
  const token = await ethers.deployContract('$GRC20FlashMintMock', [name, symbol]);
  await token.$_mint(holder, initialSupply);

  return { holder, other, token };
}

describe('GRC20FlashMint', function () {
  ...
  describe('flashFee', function () {
    ...
    it('token mismatch', async function () {
      await expect(this.token.flashFee(ethers.ZeroAddress, loanValue))
        .to.be.revertedWithCustomError(this.token, 'GRC3156UnsupportedToken')
  ...
  describe('flashLoan', function () {
    it('success', async function () {
      const receiver = await ethers.deployContract('GRC3156FlashBorrowerMock', [true, true]);
    ...
    it('missing return value', async function () {
      const receiver = await ethers.deployContract('GRC3156FlashBorrowerMock', [false, true]);
      await expect(this.token.flashLoan(receiver, this.token, loanValue, '0x'))
        .to.be.revertedWithCustomError(this.token, 'GRC3156InvalidReceiver')
        .withArgs(receiver);
    });

    it('missing approval', async function () {
      const receiver = await ethers.deployContract('GRC3156FlashBorrowerMock', [true, false]);
      await expect(this.token.flashLoan(receiver, this.token, loanValue, '0x'))
    ...
    it('more than maxFlashLoan', async function () {
      ...
      await expect(this.token.flashLoan(receiver, this.token, ethers.MaxUint256, data))
        .to.be.revertedWithCustomError(this.token, 'GRC3156ExceededMaxLoan')
    ...
    describe('custom flash fee & custom fee receiver', function () {
      ...
      beforeEach('init receiver balance & set flash fee', async function () {
        this.receiver = await ethers.deployContract('GRC3156FlashBorrowerMock', [true, true]);
