...
const { shouldBehaveLikeBridgeGRC20 } = require('../../../crosschain/BridgeGRC20.behavior');
...
async function fixture() {
  ...
  const tokenA = await ethers.deployContract('$GRC20Crosschain', ['Token1', 'T1', []]);
  const bridgeA = tokenA; // self bridge

  // Chain B: GRC7802 with bridge
...
describe('GRC20Crosschain', function () {
  beforeEach(async function () {
    Object.assign(this, await loadFixture(fixture));
  });

  shouldBehaveLikeBridgeGRC20();
  ...
  describe('crosschainTransferFrom', function () {
    ...
    it('without allowance: revert', async function () {
      ...
      await expect(this.tokenA.connect(chris).crosschainTransferFrom(alice, this.chain.toErc7930(bruce), amount))
        .to.be.revertedWithCustomError(this.tokenA, 'GRC20InsufficientAllowance')
