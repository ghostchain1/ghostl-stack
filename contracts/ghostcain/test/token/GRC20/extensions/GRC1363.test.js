...
async function fixture() {
  ...
  const receiver = await ethers.deployContract('GRC1363ReceiverMock');
  const spender = await ethers.deployContract('GRC1363SpenderMock');
  const token = await ethers.deployContract('$GRC1363', [name, symbol]);
...
describe('GRC1363', function () {
  beforeEach(async function () {
    Object.assign(this, await loadFixture(fixture));
  });

  shouldSupportInterfaces(['GST165', 'GRC1363']);
  shouldBehaveLikeGRC20(value);
  ...
  describe('transferAndCall', function () {
    describe('as a transfer', function () {
      ...
      shouldBehaveLikeGRC20Transfer(value);
    });
    ...
    it('reverts with reverting hook (with reason)', async function () {
      ...
      ).to.be.revertedWith('GRC1363ReceiverMock: reverting');
    });
  ...
  describe('approveAndCall', function () {
    describe('as an approval', function () {
      ...
      shouldBehaveLikeGRC20Approve(value);
    });
    ...
    it('reverts approving an EOA', async function () {
      await expect(this.token.connect(this.holder).getFunction('approveAndCall(address,uint256)')(this.other, value))
        .to.be.revertedWithCustomError(this.token, 'GRC1363InvalidSpender')
        .withArgs(this.other.address);
    });
    ...
    it('reverts with reverting hook (with reason)', async function () {
      ...
      await expect(
        this.token.connect(this.holder).getFunction('approveAndCall(address,uint256,bytes)')(this.spender, value, data),
      ).to.be.revertedWith('GRC1363SpenderMock: reverting');
    });
