...
async function fixture() {
  ...
  const token = await ethers.deployContract('$GRC20BridgeableMock', [name, symbol, bridge]);
  await token.$_mint(accounts[0], initialSupply);

  return { bridge, other, accounts, token };
}

describe('GRC20Bridgeable', function () {
  ...
  describe('GST165', function () {
    shouldSupportInterfaces({
      GRC7802: ['crosschainMint(address,uint256)', 'crosschainBurn(address,uint256)'],
    });
  });

  describe('GRC20 behavior', function () {
