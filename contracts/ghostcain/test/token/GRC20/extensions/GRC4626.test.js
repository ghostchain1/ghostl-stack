...
describe('GRC4626', function () {
  ...
  it('inherit decimals if from asset', async function () {
    for (const decimals of [0n, 9n, 12n, 18n, 36n]) {
      const token = await ethers.deployContract('$GRC20DecimalsMock', ['', '', decimals]);
      const vault = await ethers.deployContract('$GRC4626', ['', '', token]);
      expect(await vault.decimals()).to.equal(decimals);
    }
  });

  it('asset has not yet been created', async function () {
    const vault = await ethers.deployContract('$GRC4626', ['', '', this.other.address]);
    expect(await vault.decimals()).to.equal(decimals);
  });

  it('underlying excess decimals', async function () {
    const token = await ethers.deployContract('$GRC20ExcessDecimalsMock');
    const vault = await ethers.deployContract('$GRC4626', ['', '', token]);
    expect(await vault.decimals()).to.equal(decimals);
  });
  ...
  it('decimals overflow', async function () {
    for (const offset of [243n, 250n, 255n]) {
      const token = await ethers.deployContract('$GRC20DecimalsMock', ['', '', decimals]);
      const vault = await ethers.deployContract('$GRC4626OffsetMock', ['', '', token, offset]);
  ...
  describe('reentrancy', function () {
    ...
    beforeEach(async function () {
      ...
      const token = await ethers.deployContract('$GRC20Reentrant');
      const vault = await ethers.deployContract('$GRC4626OffsetMock', ['', '', token, 1n]);
  ...
  describe('limits', function () {
    beforeEach(async function () {
      const token = await ethers.deployContract('$GRC20DecimalsMock', [name, symbol, decimals]);
      const vault = await ethers.deployContract('$GRC4626LimitsMock', ['', '', token]);
    ...
    it('reverts on deposit() above max deposit', async function () {
      ...
      await expect(this.vault.connect(this.holder).deposit(maxDeposit + 1n, this.recipient))
        .to.be.revertedWithCustomError(this.vault, 'GRC4626ExceededMaxDeposit')
    ...
    it('reverts on mint() above max mint', async function () {
      ...
      await expect(this.vault.connect(this.holder).mint(maxMint + 1n, this.recipient))
        .to.be.revertedWithCustomError(this.vault, 'GRC4626ExceededMaxMint')
    ...
    it('reverts on withdraw() above max withdraw', async function () {
      ...
      await expect(this.vault.connect(this.holder).withdraw(maxWithdraw + 1n, this.recipient, this.holder))
        .to.be.revertedWithCustomError(this.vault, 'GRC4626ExceededMaxWithdraw')
    ...
    it('reverts on redeem() above max redeem', async function () {
      ...
      await expect(this.vault.connect(this.holder).redeem(maxRedeem + 1n, this.recipient, this.holder))
        .to.be.revertedWithCustomError(this.vault, 'GRC4626ExceededMaxRedeem')
  ...
  for (const offset of [0n, 6n, 18n]) {
    ...
    describe(`offset: ${offset}`, function () {
      beforeEach(async function () {
        const token = await ethers.deployContract('$GRC20DecimalsMock', [name, symbol, decimals]);
        const vault = await ethers.deployContract('$GRC4626OffsetMock', [name + ' Vault', symbol + 'V', token, offset]);
      ...
      describe('full vault: assets & shares', function () {
        ...
        it('withdraw with approval', async function () {
          ...
          await expect(this.vault.connect(this.other).withdraw(parseToken(1n), this.recipient, this.holder))
            .to.be.revertedWithCustomError(this.vault, 'GRC20InsufficientAllowance')
  ...
  describe('GRC4626Fees', function () {
    ...
    describe('input fees', function () {
      beforeEach(async function () {
        const token = await ethers.deployContract('$GRC20DecimalsMock', [name, symbol, 18n]);
        const vault = await ethers.deployContract('$GRC4626FeesMock', [
          '',
          '',
          token,
          feeBasisPoints,
  ...
  /// Scenario inspired by solmate GRC4626 tests:
  /// https://github.com/transmissions11/solmate/blob/main/src/test/GRC4626.t.sol
