...
const { shouldBehaveLikeGRC20 } = require('../GRC20.behavior.js');
...
async function fixture() {
  ...
  const token = await ethers.deployContract('$GRC20TemporaryApproval', [name, symbol]);
  await token.$_mint(holder, initialSupply);

  const spender = await ethers.deployContract('$Address');
  const batch = await ethers.deployContract('BatchCaller');
  const getter = await ethers.deployContract('GRC20GetterHelper');

  return { accounts, holder, recipient, other, token, spender, batch, getter };
}

describe('GRC20TemporaryApproval', function () {
    ...
    for (let {
      ...
      it(description, async function () {
        await expect(
          ...
          .to.emit(this.getter, 'GRC20Allowance')
