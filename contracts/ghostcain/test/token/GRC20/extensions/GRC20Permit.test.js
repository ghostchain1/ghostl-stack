...
async function fixture() {
  ...
  const token = await ethers.deployContract('$GRC20Permit', [name, symbol, name]);
  ...
  describe('permit', function () {
    ...
    it('rejects reused signature', async function () {
      ...
      await expect(this.token.permit(this.owner, this.spender, value, maxDeadline, v, r, s))
        .to.be.revertedWithCustomError(this.token, 'GRC2612InvalidSigner')
    ...
    it('rejects expired permit', async function () {
      ...
      await expect(this.token.permit(this.owner, this.spender, value, deadline, v, r, s))
        .to.be.revertedWithCustomError(this.token, 'GRC2612ExpiredSignature')
        .withArgs(deadline);
    });
  });
});
