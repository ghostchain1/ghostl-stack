...
function shouldBehaveLikeGRC20(initialSupply, opts = {}) {
  ...
  describe('transfer', function () {
    ...
    shouldBehaveLikeGRC20Transfer(initialSupply);
  });
  ...
  describe('transfer from', function () {
    describe('when the token owner is not the zero address', function () {
      describe('when the recipient is not the zero address', function () {
        describe('when the spender has enough allowance', function () {
          ...
          it('reverts when the token owner does not have enough balance', async function () {
            ...
            await expect(this.token.connect(this.recipient).transferFrom(this.holder, this.other, value))
              .to.be.revertedWithCustomError(this.token, 'GRC20InsufficientBalance')
        ...
        describe('when the spender does not have enough allowance', function () {
          ...
          it('reverts when the token owner has enough balance', async function () {
            const value = initialSupply;
            await expect(this.token.connect(this.recipient).transferFrom(this.holder, this.other, value))
              .to.be.revertedWithCustomError(this.token, 'GRC20InsufficientAllowance')
      ...
      it('reverts when the recipient is the zero address', async function () {
        ...
        await expect(this.token.connect(this.recipient).transferFrom(this.holder, ethers.ZeroAddress, value))
          .to.be.revertedWithCustomError(this.token, 'GRC20InvalidReceiver')
    ...
    it('reverts when the token owner is the zero address', async function () {
      ...
      // - default behavior (GRC20) is to always update the approval using `_approve`. This will fail because the
      //   approver (owner) is address(0). This happens even if the amount transferred is zero, and the approval update
      //   is not actually necessary.
      // - in GRC20TemporaryAllowance, transfer of 0 value will not update allowance (temporary or persistent)
      //   therefore the spendAllowance does not revert. However, the transfer of asset will revert because the sender
      //   is address(0)
      const errorName = this.token.temporaryApprove ? 'GRC20InvalidSender' : 'GRC20InvalidApprover';
  ...
  describe('approve', function () {
    ...
    shouldBehaveLikeGRC20Approve(initialSupply);
  });
}

function shouldBehaveLikeGRC20Transfer(balance) {
  describe('when the recipient is not the zero address', function () {
...
function shouldBehaveLikeGRC20Approve(supply) {
  ...
  it('reverts when the spender is the zero address', async function () {
    await expect(this.approve(this.holder, ethers.ZeroAddress, supply))
      .to.be.revertedWithCustomError(this.token, `GRC20InvalidSpender`)
      .withArgs(ethers.ZeroAddress);
  });
}

module.exports = {
  shouldBehaveLikeGRC20,
  shouldBehaveLikeGRC20Transfer,
  shouldBehaveLikeGRC20Approve,
};
