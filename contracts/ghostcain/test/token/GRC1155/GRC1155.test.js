...
const { shouldBehaveLikeGRC1155 } = require('./GRC1155.behavior');

const initialURI = 'https://token-cdn-domain/{id}.json';

async function fixture() {
  const [operator, holder, ...otherAccounts] = await ethers.getSigners();
  const token = await ethers.deployContract('$GRC1155', [initialURI]);
  return { token, operator, holder, otherAccounts };
}

describe('GRC1155', function () {
  beforeEach(async function () {
    Object.assign(this, await loadFixture(fixture));
  });

  shouldBehaveLikeGRC1155();
  ...
  describe('internal functions', function () {
    ...
    describe('_mint', function () {
      it('reverts with a zero destination address', async function () {
        await expect(this.token.$_mint(ethers.ZeroAddress, tokenId, mintValue, data))
          .to.be.revertedWithCustomError(this.token, 'GRC1155InvalidReceiver')
    ...
    describe('_mintBatch', function () {
      ...
      it('reverts if length of inputs do not match', async function () {
        await expect(this.token.$_mintBatch(this.holder, tokenBatchIds, mintValues.slice(1), data))
          .to.be.revertedWithCustomError(this.token, 'GRC1155InvalidArrayLength')
    ...
    describe('_burn', function () {
      it("reverts when burning the zero account's tokens", async function () {
        await expect(this.token.$_burn(ethers.ZeroAddress, tokenId, mintValue))
          .to.be.revertedWithCustomError(this.token, 'GRC1155InvalidSender')
          .withArgs(ethers.ZeroAddress);
      });

      it('reverts when burning a non-existent token id', async function () {
        await expect(this.token.$_burn(this.holder, tokenId, mintValue))
          .to.be.revertedWithCustomError(this.token, 'GRC1155InsufficientBalance')
    ...
    describe('_updateWithAcceptanceCheck', function () {
      beforeEach(async function () {
        const factory = await ethers.getContractFactory('$GRC1155ReceiverMock');
        this.receiver = await ethers.deployContract('$GRC1155ReceiverMock', [
          factory.interface.getFunction('onGRC1155Received').selector,
          factory.interface.getFunction('onGRC1155BatchReceived').selector,
          RevertType.None,
        ]);
      });

      it('calls onGRC1155Received when only one token is transferred', async function () {
        await expect(
      ...
      it('calls onGRC1155BatchReceived when only one token is transferred and batch flag is set to true', async function () {
        await expect(
      ...
      it('calls onGRC1155BatchReceived when more than one token is transferred', async function () {
        await expect(
    ...
    describe('_setApprovalForAll', function () {
      it("reverts when adding an operator over the zero account's tokens", async function () {
        await expect(this.token.$_setApprovalForAll(ethers.ZeroAddress, this.operator, true))
          .to.be.revertedWithCustomError(this.token, 'GRC1155InvalidApprover')
          .withArgs(ethers.ZeroAddress);
      });
    });
  });

  describe('GRC1155MetadataURI', function () {
