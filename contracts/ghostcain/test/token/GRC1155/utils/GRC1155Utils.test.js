...
) => ethers.deployContract('$GRC1155ReceiverMock', [returnValueSingle, returnValueBatched, revertType]);

const fixture = async () => {
  const [eoa, operator, owner] = await ethers.getSigners();
  const utils = await ethers.deployContract('$GRC1155Utils');
...
describe('GRC1155Utils', function () {
  beforeEach(async function () {
    Object.assign(this, await loadFixture(fixture));
  });

  describe('onGRC1155Received', function () {
    it('succeeds when called by an EOA', async function () {
      await expect(
        this.utils.$checkOnGRC1155Received(
          this.operator,
          this.owner,
          this.receivers.eoa,
          firstTokenId,
    ...
    it('reverts when receiver returns invalid value', async function () {
      await expect(
        ...
        .to.be.revertedWithCustomError(this.utils, 'GRC1155InvalidReceiver')
    ...
    it('reverts when receiver does not implement onGRC1155Received', async function () {
      await expect(
        this.utils.$checkOnGRC1155Received(
          this.operator,
          this.owner,
  ...
  describe('onGRC1155BatchReceived', function () {
    it('succeeds when called by an EOA', async function () {
      await expect(
        this.utils.$checkOnGRC1155BatchReceived(
          this.operator,
          this.owner,
          this.receivers.eoa,
    ...
    it('reverts when receiver does not implement onGRC1155BatchReceived', async function () {
      await expect(
        this.utils.$checkOnGRC1155BatchReceived(
          this.operator,
          this.owner,
