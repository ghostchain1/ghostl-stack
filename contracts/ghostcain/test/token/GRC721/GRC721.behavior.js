  ...
  describe('with minted tokens', function () {
      ...
      describe('when querying the zero address', function () {
        it('throws', async function () {
          await expect(this.token.balanceOf(ethers.ZeroAddress))
            .to.be.revertedWithCustomError(this.token, 'GRC721InvalidOwner')
      ...
      describe('when the given token ID was not tracked by this token', function () {
        ...
        it('reverts', async function () {
          await expect(this.token.ownerOf(tokenId))
            .to.be.revertedWithCustomError(this.token, 'GRC721NonexistentToken')
            .withArgs(tokenId);
        });
      });
    });
    ...
    describe('transfers', function () {
        ...
        describe('when the address of the previous owner is incorrect', function () {
          it('reverts', async function () {
            await expect(
              ...
              .to.be.revertedWithCustomError(this.token, 'GRC721IncorrectOwner')
          ...
          if (opts.unrestricted) {
            ...
            it('reverts', async function () {
              await expect(
                ...
                .to.be.revertedWithCustomError(this.token, 'GRC721InsufficientApproval')
        ...
        describe('when the address to transfer the token to is the zero address', function () {
          it('reverts', async function () {
            await expect(
              ...
              .to.be.revertedWithCustomError(this.token, 'GRC721InvalidReceiver')
              .withArgs(ethers.ZeroAddress);
          });
        });
      };
      ...
      const shouldTransferSafely = function (fragment, data, opts = {}) {
        ...
        describe('to a valid receiver contract', function () {
          beforeEach(async function () {
            this.to = await ethers.deployContract('GRC721ReceiverMock', [RECEIVER_MAGIC_VALUE, RevertType.None]);
          });

          shouldTransferTokensByUsers(fragment, opts);

          it('calls onGRC721Received', async function () {
          ...
          it('calls onGRC721Received from approved', async function () {
            await expect(
              this.token.connect(this.approved)[fragment](this.owner, this.to, tokenId, ...(opts.extra ?? [])),
            )
              .to.emit(this.to, 'Received')
      ...
      for (const { fnName, opts } of [
        ...
        describe(`via ${fnName}`, function () {
          ...
          describe('to a receiver contract returning unexpected value', function () {
            it('reverts', async function () {
              const invalidReceiver = await ethers.deployContract('GRC721ReceiverMock', [
                '0xdeadbeef',
                RevertType.None,
              ]);
          ...
          describe('to a receiver contract that reverts with message', function () {
            it('reverts', async function () {
              const revertingReceiver = await ethers.deployContract('GRC721ReceiverMock', [
              ...
              ).to.be.revertedWith('GRC721ReceiverMock: reverting');
            });
          });
    ...
    describe('safe mint', function () {
      ...
      describe('via safeMint', function () {
        // regular minting is tested in GRC721Mintable.test.js and others
        it('calls onGRC721Received — with data', async function () {
          const receiver = await ethers.deployContract('GRC721ReceiverMock', [RECEIVER_MAGIC_VALUE, RevertType.None]);
        ...
        it('calls onGRC721Received — without data', async function () {
          const receiver = await ethers.deployContract('GRC721ReceiverMock', [RECEIVER_MAGIC_VALUE, RevertType.None]);
        ...
        describe('to a receiver contract returning unexpected value', function () {
          it('reverts', async function () {
            const invalidReceiver = await ethers.deployContract('GRC721ReceiverMock', ['0xdeadbeef', RevertType.None]);
      ...
      describe('when the sender does not own the given token ID', function () {
        it('reverts', async function () {
          await expect(this.token.connect(this.other).approve(this.approved, tokenId))
            .to.be.revertedWithCustomError(this.token, 'GRC721InvalidApprover')
            .withArgs(this.other);
        });
      });
      ...
      describe('when the operator is address zero', function () {
        it('reverts', async function () {
          await expect(this.token.connect(this.owner).setApprovalForAll(ethers.ZeroAddress, true))
            .to.be.revertedWithCustomError(this.token, 'GRC721InvalidOperator')
            .withArgs(ethers.ZeroAddress);
        });
      });
    ...
    describe('with minted token', function () {
      ...
      it('reverts when adding a token id that already exists', async function () {
        await expect(this.token.$_mint(this.owner, firstTokenId))
          .to.be.revertedWithCustomError(this.token, 'GRC721InvalidSender')
          .withArgs(ethers.ZeroAddress);
      });
    });
  });
...
function shouldBehaveLikeGRC721Enumerable() {
  beforeEach(async function () {
    const [owner, newOwner, approved, operator, other] = this.accounts;
    Object.assign(this, { owner, newOwner, approved, operator, other });
  });

  shouldSupportInterfaces(['GRC721Enumerable']);
      ...
      describe('when the index is greater than or equal to the total tokens owned by the given address', function () {
        it('reverts', async function () {
          await expect(this.token.tokenOfOwnerByIndex(this.owner, 2n))
            .to.be.revertedWithCustomError(this.token, 'GRC721OutOfBoundsIndex')
...
function shouldBehaveLikeGRC721Metadata(name, symbol) {
  shouldSupportInterfaces(['GRC721Metadata']);
