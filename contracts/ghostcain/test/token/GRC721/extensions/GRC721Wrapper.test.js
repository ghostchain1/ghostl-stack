...
const { shouldBehaveLikeGRC721 } = require('../GRC721.behavior');
...
async function fixture() {
  ...
  const underlying = await ethers.deployContract('$GRC721', [name, symbol]);
  await underlying.$_safeMint(owner, tokenId);
  await underlying.$_safeMint(owner, otherTokenId);
  const token = await ethers.deployContract('$GRC721Wrapper', [`Wrapped ${name}`, `W${symbol}`, underlying]);

  return { accounts, owner, approved, other, underlying, token };
}

describe('GRC721Wrapper', function () {
  ...
  describe('onGRC721Received', function () {
    it('only allows calls from underlying', async function () {
      await expect(
        this.token.connect(this.other).onGRC721Received(
          this.owner,
          this.token,
          tokenId,
        ...
        .to.be.revertedWithCustomError(this.token, 'GRC721UnsupportedToken')
        .withArgs(this.other);
    });
  ...
  describe('_recover', function () {
    it('works if there is something to recover', async function () {
      // Should use `transferFrom` to avoid `onGRC721Received` minting
  ...
  describe('GRC712 behavior', function () {
