...
describe('GRC721Consecutive', function () {
  for (const offset of [0n, 1n, 42n]) {
    describe(`with offset ${offset}`, function () {
      async function fixture() {
        ...
        const token = await ethers.deployContract('$GRC721ConsecutiveMock', [
          name,
          symbol,
          offset,
          delegates,
      ...
      describe('minting during construction', function () {
        ...
        it('reverts on consecutive minting to the zero address', async function () {
          await expect(
            ethers.deployContract('$GRC721ConsecutiveMock', [
              name,
              symbol,
              offset,
              this.delegates,
      ...
      describe('GRC721 behavior', function () {
  ...
  describe('invalid use', function () {
    ...
    it('cannot mint a batch larger than 5000', async function () {
      const factory = await ethers.getContractFactory('$GRC721ConsecutiveMock');

      await expect(ethers.deployContract('$GRC721ConsecutiveMock', [name, symbol, 0, [], [receiver], [5001n]]))
        .to.be.revertedWithCustomError(factory, 'GRC721ExceededMaxBatchMint')
        .withArgs(5001n, 5000n);
    });

    it('cannot use single minting during construction', async function () {
      const factory = await ethers.getContractFactory('$GRC721ConsecutiveNoConstructorMintMock');

      await expect(
        ethers.deployContract('$GRC721ConsecutiveNoConstructorMintMock', [name, symbol]),
      ).to.be.revertedWithCustomError(factory, 'GRC721ForbiddenMint');
    });

    it('consecutive mint not compatible with enumerability', async function () {
      const factory = await ethers.getContractFactory('$GRC721ConsecutiveEnumerableMock');

      await expect(
        ethers.deployContract('$GRC721ConsecutiveEnumerableMock', [name, symbol, [receiver], [100n]]),
      ).to.be.revertedWithCustomError(factory, 'GRC721EnumerableForbiddenBatchMint');
    });
  });
});
