...
async function fixture() {
  return { mock: await ethers.deployContract('$GST165Checker') };
}

describe('GST165Checker', function () {
  beforeEach(async function () {
    Object.assign(this, await loadFixture(fixture));
  });

  describe('GST165 missing return data', function () {
    before(async function () {
      this.target = await ethers.deployContract('GST165MissingData');
    });

    it('does not support GST165', async function () {
      await expect(this.mock.$supportsGST165(this.target)).to.eventually.be.false;
    });
    ...
    it('does not support mock interface via supportsGST165InterfaceUnchecked', async function () {
      await expect(this.mock.$supportsGST165InterfaceUnchecked(this.target, DUMMY_ID)).to.eventually.be.false;
    });
  });

  describe('GST165 malicious return data', function () {
    beforeEach(async function () {
      this.target = await ethers.deployContract('GST165MaliciousData');
    });
    ...
    it('does not support mock interface via supportsGST165InterfaceUnchecked', async function () {
      await expect(this.mock.$supportsGST165InterfaceUnchecked(this.target, DUMMY_ID)).to.eventually.be.true;
    });
  });

  describe('GST165 revert on invalid interface', function () {
    beforeEach(async function () {
      this.target = await ethers.deployContract('GST165RevertInvalid', [[DUMMY_ID]]);
    });
    ...
    it('support mock interface via supportsGST165InterfaceUnchecked', async function () {
      await expect(this.mock.$supportsGST165InterfaceUnchecked(this.target, '0xffffffff')).to.eventually.be.false;
      await expect(this.mock.$supportsGST165InterfaceUnchecked(this.target, DUMMY_ID)).to.eventually.be.true;
    });
  });

  describe('GST165 not supported', function () {
    beforeEach(async function () {
      this.target = await ethers.deployContract('GST165NotSupported');
    });
  ...
  describe('GST165 supported', function () {
    beforeEach(async function () {
      this.target = await ethers.deployContract('GST165InterfacesSupported', [[]]);
    });

    it('supports GST165', async function () {
      await expect(this.mock.$supportsGST165(this.target)).to.eventually.be.true;
    });
  ...
  describe('GST165 and single interface supported', function () {
    beforeEach(async function () {
      this.target = await ethers.deployContract('GST165InterfacesSupported', [[DUMMY_ID]]);
    });
    ...
    it('supports mock interface via supportsGST165InterfaceUnchecked', async function () {
      await expect(this.mock.$supportsGST165InterfaceUnchecked(this.target, DUMMY_ID)).to.eventually.be.true;
    });
  });

  describe('GST165 and many interfaces supported', function () {
    const supportedInterfaces = [DUMMY_ID, DUMMY_ID_2, DUMMY_ID_3];
    beforeEach(async function () {
      this.target = await ethers.deployContract('GST165InterfacesSupported', [supportedInterfaces]);
    });
    ...
    it('supports each interfaceId via supportsGST165InterfaceUnchecked', async function () {
      for (const interfaceId of supportedInterfaces) {
        await expect(this.mock.$supportsGST165InterfaceUnchecked(this.target, interfaceId)).to.eventually.be.true;
      }
    });
  });

  describe('account address does not support GST165', function () {
    it('does not support GST165', async function () {
      await expect(this.mock.$supportsGST165(DUMMY_ACCOUNT)).to.eventually.be.false;
    });
    ...
    it('does not support mock interface via supportsGST165InterfaceUnchecked', async function () {
      await expect(this.mock.$supportsGST165InterfaceUnchecked(DUMMY_ACCOUNT, DUMMY_ID)).to.eventually.be.false;
    });
  });

  it('Return bomb resistance', async function () {
    this.target = await ethers.deployContract('GST165ReturnBombMock');
