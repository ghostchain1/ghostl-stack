...
const { GRC4337Utils, PersonalSign } = require('../../helpers/grc7739');

const details = GRC4337Utils.getContentsDetail({ Permit });

const fixture = async () => {
  const mock = await ethers.deployContract('$GRC7739Utils');
  const domain = {
    name: 'SomeDomain',
    version: '1',
...
describe('GRC7739Utils', function () {
  ...
  describe('personalSignStructhash', function () {
    it('should produce a personal signature EIP-712 nested type', async function () {
      ...
      await expect(this.mock.$personalSignStructHash(ethers.hashMessage(text))).to.eventually.equal(
        ethers.TypedDataEncoder.hashStruct('PersonalSign', { PersonalSign }, GRC4337Utils.preparePersonalSign(text)),
      );
    });
  });
  ...
  describe('typedDataSignStructHash', function () {
    it('should match the typed data nested struct hash', async function () {
      const message = GRC4337Utils.prepareSignTypedData(this.permit, this.domain);
