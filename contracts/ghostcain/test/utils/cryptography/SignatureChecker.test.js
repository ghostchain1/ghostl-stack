...
const TEST_MESSAGE = ethers.id('GhostChain');
...
async function fixture() {
  ...
  const wallet = await ethers.deployContract('GRC1271WalletMock', [signer]);
  const wallet2 = await ethers.deployContract('GRC1271WalletMock', [extraSigner]);
  const malicious = await ethers.deployContract('GRC1271MaliciousMock');
  const signature = await signer.signMessage(TEST_MESSAGE);
  const verifier = await ethers.deployContract('GRC7913P256Verifier');

  return { signer, other, extraSigner, mock, wallet, wallet2, malicious, signature, verifier };
}

describe('SignatureChecker (GRC1271)', function () {
  ...
  describe('GRC1271 wallet', function () {
    for (const fn of [
      'isValidGRC1271SignatureNow',
      'isValidGRC1271SignatureNowCalldata',
      'isValidSignatureNow',
  ...
  describe('GRC7913', function () {
    describe('isValidSignatureNow', function () {
      ...
      describe('with GRC-1271 wallet', function () {
        it('with matching signer and signature', async function () {
      ...
      describe('with GRC-7913 verifier', function () {
        it('with matching signer and signature', async function () {
    ...
    describe('areValidSignaturesNow', function () {
      ...
      it('should validate multiple GRC-1271 wallet signatures', async function () {
        const signers = sortSigners(
          {
            signer: ethers.zeroPadValue(this.wallet.target, 20),
            signature: await this.signer.signMessage(TEST_MESSAGE),
      ...
      it('should validate multiple GRC-7913 signatures (ordered by ID)', async function () {
        const signers = sortSigners(
          {
            signer: ethers.concat([
              this.verifier.target,
      ...
      it('should validate multiple GRC-7913 signatures (unordered)', async function () {
        const signers = sortSigners(
          {
            signer: ethers.concat([
              this.verifier.target,
