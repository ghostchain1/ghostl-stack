...
class GRC7739Signer extends ethers.AbstractSigner {
  #signer;
  #domain;
  ...
  async signMessage(message) {
    return this.#signer.signTypedData(this.#domain, { PersonalSign }, GRC4337Utils.preparePersonalSign(message));
  }

  async signTypedData(domain, types, value) {
    const { allTypes, contentsTypeName, contentsDescr } = GRC4337Utils.getContentsDetail(types);

    return Promise.resolve(
      this.#signer.signTypedData(domain, allTypes, GRC4337Utils.prepareSignTypedData(value, this.#domain)),
    ).then(signature =>
      ethers.concat([
        signature,
