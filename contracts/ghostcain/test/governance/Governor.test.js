...
const { shouldBehaveLikeGRC6372 } = require('./utils/GRC6372.behavior');

const TOKENS = [
  { Token: '$GRC20Votes', mode: 'blocknumber' },
  { Token: '$GRC20VotesTimestampMock', mode: 'timestamp' },
  { Token: '$GRC20VotesLegacyMock', mode: 'blocknumber' },
];
...
async function deployToken(contractName) {
  try {
    ...
    if (error.message == 'incorrect number of arguments to constructor') {
      // GRC20VotesLegacyMock has a different construction that uses version='1' by default.
...
describe('Governor', function () {
  for (const { Token, mode } of TOKENS) {
    ...
    describe(`using ${Token}`, function () {
      ...
      shouldSupportInterfaces(['GRC1155Receiver', 'Governor', 'Governor_5_3']);
      shouldBehaveLikeGRC6372(mode);
      ...
      describe('vote with signature', function () {
        ...
        it('votes with a valid EIP-1271 signature', async function () {
          const wallet = await ethers.deployContract('GRC1271WalletMock', [this.userEOA]);
      ...
      describe('safe receive', function () {
        describe('GRC721', function () {
          ...
          beforeEach(async function () {
            this.token = await ethers.deployContract('$GRC721', ['Non Fungible Token', 'NFT']);
            await this.token.$_mint(this.owner, tokenId);
          });

          it('can receive an GRC721 safeTransfer', async function () {
        ...
        describe('GRC1155', function () {
          ...
          beforeEach(async function () {
            this.token = await ethers.deployContract('$GRC1155', ['https://token-cdn-domain/{id}.json']);
            await this.token.$_mintBatch(this.owner, Object.keys(tokenIds), Object.values(tokenIds), '0x');
          });

          it('can receive GRC1155 safeTransfer', async function () {
          ...
          it('can receive GRC1155 safeBatchTransfer', async function () {
            await this.token
              .connect(this.owner)
