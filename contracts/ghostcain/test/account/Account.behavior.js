...
const { SIG_VALIDATION_SUCCESS, SIG_VALIDATION_FAILURE } = require('../helpers/grc4337');
...
function shouldBehaveLikeAccountHolder() {
  describe('onReceived', function () {
    ...
    shouldSupportInterfaces(['GRC1155Receiver']);
    ...
    describe('onGRC1155Received', function () {
      ...
      beforeEach(async function () {
        this.token = await ethers.deployContract('$GRC1155', ['https://somedomain.com/{id}.json']);
        await this.token.$_mintBatch(this.other, ids, values, '0x');
      });

      it('receives GRC1155 tokens from a single ID', async function () {
      ...
      it('receives GRC1155 tokens from a multiple IDs', async function () {
        await expect(
          this.token.balanceOfBatch(
            ids.map(() => this.mock),
            ids,
    ...
    describe('onGRC721Received', function () {
      ...
      beforeEach(async function () {
        this.token = await ethers.deployContract('$GRC721', ['Some NFT', 'SNFT']);
