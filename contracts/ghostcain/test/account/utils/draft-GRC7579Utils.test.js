...
const fixture = async () => {
  ...
  const utils = await ethers.deployContract('$GRC7579Utils', { value: ethers.parseEther('1') });
  const utilsGlobal = await ethers.deployContract('$GRC7579UtilsGlobalMock');
...
describe('GRC7579Utils', function () {
  ...
  describe('execSingle', function () {
    ...
    it('default to calling self is target is address(0) (GRC-7821 calldata compression)', async function () {
      const data = encodeSingle(
    ...
    it('emits GRC7579TryExecuteFail event when target reverts in try ExecType', async function () {
      ...
      await expect(this.utils.$execSingle(data, EXEC_TYPE_TRY))
        .to.emit(this.utils, 'GRC7579TryExecuteFail')
        .withArgs(
          CALL_TYPE_CALL,
    ...
    it('reverts with an invalid exec type', async function () {
      ...
      await expect(this.utils.$execSingle(data, '0x03'))
        .to.be.revertedWithCustomError(this.utils, 'GRC7579UnsupportedExecType')
        .withArgs('0x03');
    });
  });
