...
import {Ownable} from "@ghostchain/contracts/access/Ownable.sol";
...
import {PackedUserOperation, IAccount, IEntryPoint} from "@ghostchain/contracts/interfaces/draft-IGRC4337.sol";
import {GRC4337Utils} from "@ghostchain/contracts/account/utils/draft-GRC4337Utils.sol";
import {
    GRC7579Utils,
    Mode,
    CallType,
...
contract SampleAccount is IAccount, Ownable {
    ...
    function validateUserOp(
        ...
        require(msg.sender == address(GRC4337Utils.ENTRYPOINT_V07), "only from EP");
        // Check signature
        ...
        if (bytes4(userOp.callData[0x00:0x04]) == this.execute.selector) {
            ...
            if (callType == GRC7579Utils.CALLTYPE_BATCH) {
                // Remove the selector
        ...
        return GRC4337Utils.SIG_VALIDATION_SUCCESS;
    }

    function execute(Mode mode, bytes calldata executionCalldata) external payable {
        require(msg.sender == address(this) || msg.sender == address(GRC4337Utils.ENTRYPOINT_V07), "not auth");

        (CallType callType, ExecType execType, , ) = mode.decodeMode();

        // check if calltype is batch or single
        if (callType == GRC7579Utils.CALLTYPE_SINGLE) {
            executionCalldata.execSingle(execType);
        } else if (callType == GRC7579Utils.CALLTYPE_BATCH) {
            executionCalldata.execBatch(execType);

            emit Log(false, executionCalldata.decodeBatch());
        } else if (callType == GRC7579Utils.CALLTYPE_DELEGATECALL) {
...
contract GRC7579UtilsTest is Test {
    using MessageHashUtils for *;
    using GRC4337Utils for *;
    using GRC7579Utils for *;
    ...
    function testExecuteBatchDecodeCorrectly() public {
        ...
        userOps[0] = PackedUserOperation({
            ...
            callData: abi.encodeCall(
                ...
                (
                    GRC7579Utils.encodeMode(
                        GRC7579Utils.CALLTYPE_BATCH,
                        GRC7579Utils.EXECTYPE_DEFAULT,
                        ModeSelector.wrap(0x00),
                        ModePayload.wrap(0x00)
                    ),
                    GRC7579Utils.encodeBatch(calls)
                )
            ),
        ...
        vm.recordLogs();
        GRC4337Utils.ENTRYPOINT_V07.handleOps(userOps, payable(_beneficiary));
    ...
    function testExecuteBatchDecodeEmpty() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                ...
                0,
                "AA23 reverted",
                abi.encodeWithSelector(GRC7579Utils.GRC7579DecodingError.selector)
            )
        );
        GRC4337Utils.ENTRYPOINT_V07.handleOps(userOps, payable(_beneficiary));
    ...
    function testDecodeBatch() public {
        // BAD: buffer empty
        vm.expectRevert(GRC7579Utils.GRC7579DecodingError.selector);
