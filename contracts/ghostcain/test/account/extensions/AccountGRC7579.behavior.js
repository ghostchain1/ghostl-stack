...
function shouldBehaveLikeAccountGRC7579({ withHooks = false } = {}) {
  describe('AccountGRC7579', function () {
    beforeEach(async function () {
      ...
      this.modules = {};
      this.modules[MODULE_TYPE_VALIDATOR] = await ethers.deployContract('$GRC7579ModuleMock', [MODULE_TYPE_VALIDATOR]);
      this.modules[MODULE_TYPE_EXECUTOR] = await ethers.deployContract('$GRC7579ModuleMock', [MODULE_TYPE_EXECUTOR]);
      this.modules[MODULE_TYPE_FALLBACK] = await ethers.deployContract('$GRC7579ModuleMock', [MODULE_TYPE_FALLBACK]);
      this.modules[MODULE_TYPE_HOOK] = await ethers.deployContract('$GRC7579HookMock');
    ...
    describe('accountId', function () {
      it('should return the account ID', async function () {
        await expect(this.mock.accountId()).to.eventually.equal(
          withHooks
            ? '@ghostchain/contracts.AccountGRC7579Hooked.v1.0.0'
            : '@ghostchain/contracts.AccountGRC7579.v1.0.0',
        );
      });
    });
    ...
    describe('module installation', function () {
      ...
      it('should revert if the module type is not supported', async function () {
        await expect(this.mockFromEntrypoint.installModule(MODULE_TYPE_INVALID, this.mock, '0x'))
          .to.be.revertedWithCustomError(this.mock, 'GRC7579UnsupportedModuleType')
      ...
      it('should revert if the module is not the provided type', async function () {
        ...
        await expect(this.mockFromEntrypoint.installModule(MODULE_TYPE_VALIDATOR, instance, '0x'))
          .to.be.revertedWithCustomError(this.mock, 'GRC7579MismatchedModuleTypeId')
        ...
        it(`does not allow to install a module of ${moduleTypeId} id twice`, async function () {
          ...
          await expect(this.mockFromEntrypoint.installModule(moduleTypeId, instance, fullData))
            .to.be.revertedWithCustomError(
              this.mock,
              moduleTypeId == MODULE_TYPE_HOOK ? 'GRC7579HookModuleAlreadyPresent' : 'GRC7579AlreadyInstalledModule',
            )
      ...
      it('should revert when installing a fallback module with an initData that is not long enough to encode a function selector', async function () {
        ...
        ).to.be.revertedWithCustomError(this.mock, 'GRC7579CannotDecodeFallbackData');
    ...
    describe('module uninstallation', function () {
      ...
      for (const moduleTypeId of [
        ...
        it(`should revert uninstalling a module of type ${moduleTypeId} if it was not installed`, async function () {
          ...
          await expect(this.mockFromEntrypoint.uninstallModule(moduleTypeId, instance, fullData))
            .to.be.revertedWithCustomError(this.mock, 'GRC7579UninstalledModule')
            .withArgs(moduleTypeId, instance);
        });
      }
      ...
      it('should revert uninstalling a module of type MODULE_TYPE_FALLBACK if a different module was installed for the provided selector', async function () {
        const instance = this.modules[MODULE_TYPE_FALLBACK];
        const anotherInstance = await ethers.deployContract('$GRC7579ModuleMock', [MODULE_TYPE_FALLBACK]);
      ...
      it('should uninstall a module even if its onUninstall hook reverts', async function () {
        const maliciousModule = await ethers.deployContract('$GRC7579ModuleMaliciousMock', [MODULE_TYPE_EXECUTOR]);
        ...
        describe(`executing with ${execFn}`, function () {
          it('should revert if the call type is not supported', async function () {
            await expect(
              ...
              .to.be.revertedWithCustomError(this.mock, 'GRC7579UnsupportedCallType')
          ...
          it('should revert if the caller is not authorized / installed', async function () {
            const error = execFn == 'execute' ? 'AccountUnauthorized' : 'GRC7579UninstalledModule';
          ...
          describe('single execution', function () {
            ...
            it('emits GRC7579TryExecuteFail event when target reverts in try ExecType', async function () {
              const value = 0x012;
              const data = encodeSingle(
                this.target,
                value,
              ...
              await expect(this[mock][execFn](encodeMode({ callType: CALL_TYPE_CALL, execType: EXEC_TYPE_TRY }), data))
                .to.emit(this.mock, 'GRC7579TryExecuteFail')
                .withArgs(
                  CALL_TYPE_CALL,
          ...
          describe('batch execution', function () {
            ...
            it('emits GRC7579TryExecuteFail event when any target reverts in try ExecType', async function () {
              const value1 = 0x012;
              const value2 = 0x234;
              const data = encodeBatch(
                [this.target, value1, this.target.interface.encodeFunctionData('mockFunction')],
    ...
    describe('fallback', function () {
      beforeEach(async function () {
        this.fallbackHandler = await ethers.deployContract('$GRC7579FallbackHandlerMock');
      });
      ...
      it('reverts if there is no fallback module installed', async function () {
        ...
        await expect(this.fallbackHandler.attach(this.mock).callPayable())
          .to.be.revertedWithCustomError(this.mock, 'GRC7579MissingFallbackHandler')
          .withArgs(selector);
      });
      ...
      describe('with a fallback module installed', function () {
        ...
        it('forwards the call to the fallback handler', async function () {
          ...
          await expect(this.fallbackHandler.attach(this.mock).connect(this.other).callPayable({ value }))
            .to.emit(this.fallbackHandler, 'GRC7579FallbackHandlerMockCalled')
        ...
        it('bubble up reverts from the fallback handler', async function () {
          ...
          ).to.be.revertedWithCustomError(this.fallbackHandler, 'GRC7579FallbackHandlerMockRevert');
        });
