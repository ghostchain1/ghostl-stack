...
async function fixture() {
  ...
  // Sign user operations using MultiGRC7913SigningKey
...
describe('AccountMultiSigner', function () {
  ...
  describe('Multi ECDSA signers with threshold=1', function () {
    beforeEach(async function () {
      this.signer = new NonNativeSigner(new MultiGRC7913SigningKey([signerECDSA1]));
  ...
  describe('Multi ECDSA signers with threshold=2', function () {
    beforeEach(async function () {
      this.signer = new NonNativeSigner(new MultiGRC7913SigningKey([signerECDSA1, signerECDSA2]));
  ...
  describe('Mixed signers with threshold=2', function () {
    beforeEach(async function () {
      ...
      this.signer = new NonNativeSigner(new MultiGRC7913SigningKey([signerECDSA1, signerP256, signerRSA]));
  ...
  describe('Signer management', function () {
    ...
    it('can add signers', async function () {
      ...
      await expect(this.mock.$_addSigners(signers))
        .to.emit(this.mock, 'GRC7913SignerAdded')
      ...
      await expect(this.mock.$_addSigners(signers))
        .to.be.revertedWithCustomError(this.mock, 'MultiSignerGRC7913AlreadyExists')
    ...
    it('can change threshold', async function () {
      ...
      await expect(this.mock.$_setThreshold(2)).to.emit(this.mock, 'GRC7913ThresholdSet');
      ...
      await expect(this.mock.$_setThreshold(0)).to.be.revertedWithCustomError(
        this.mock,
        'MultiSignerGRC7913ZeroThreshold',
      );
    });
    ...
    it('rejects invalid signer format', async function () {
      ...
      await expect(this.mock.$_addSigners([invalidSigner]))
        .to.be.revertedWithCustomError(this.mock, 'MultiSignerGRC7913InvalidSigner')
