...
const { NonNativeSigner, P256SigningKey, RSASHA256SigningKey, MultiGRC7913SigningKey } = require('../helpers/signers');
...
async function fixture() {
  ...
  // Sign user operations using NonNativeSigner with MultiGRC7913SigningKey
...
describe('AccountMultiSignerWeighted', function () {
  ...
  describe('Weighted signers with equal weights (1, 1, 1) and threshold=2', function () {
    beforeEach(async function () {
      this.signer = new NonNativeSigner(new MultiGRC7913SigningKey([signerECDSA1, signerECDSA3])); // 2 accounts, weight 1+1=2
  ...
  describe('Weighted signers with varying weights (1, 2, 3) and threshold=3', function () {
    beforeEach(async function () {
      this.signer = new NonNativeSigner(new MultiGRC7913SigningKey([signerECDSA1, signerECDSA2])); // 2 accounts, weight 1+2=3
  ...
  describe('Mixed weighted signers with threshold=4', function () {
    beforeEach(async function () {
      ...
      this.signer = new NonNativeSigner(new MultiGRC7913SigningKey([signerP256, signerRSA])); // 2 accounts, weight 2+3=5
  ...
  describe('Weight management', function () {
    ...
    it('can update signer weights', async function () {
      ...
      await expect(this.mock.$_setSignerWeights([signer1, signer2], [5, 6]))
        .to.emit(this.mock, 'GRC7913SignerWeightChanged')
        .withArgs(signer1, 5)
        .to.emit(this.mock, 'GRC7913SignerWeightChanged')
        .withArgs(signer2, 6);
    ...
    it("no-op doesn't emit an event", async function () {
      await expect(this.mock.$_setSignerWeights([signer1], [1])).to.not.emit(this.mock, 'GRC7913SignerWeightChanged');
    });
    ...
    it('cannot set weight to non-existent signer', async function () {
      ...
      await expect(this.mock.$_setSignerWeights([signer4], [1]))
        .to.be.revertedWithCustomError(this.mock, 'MultiSignerGRC7913NonexistentSigner')
    ...
    it('cannot set weight to 0', async function () {
      ...
      await expect(this.mock.$_setSignerWeights([signer1], [0]))
        .to.be.revertedWithCustomError(this.mock, 'MultiSignerGRC7913WeightedInvalidWeight')
    ...
    it('requires signers and weights arrays to have same length', async function () {
      ...
      await expect(this.mock.$_setSignerWeights([signer1, signer2], [1])).to.be.revertedWithCustomError(
        this.mock,
        'MultiSignerGRC7913WeightedMismatchedLength',
      );
    ...
    it('validates threshold is reachable when updating weights', async function () {
      ...
      await expect(this.mock.$_setThreshold(9)).to.emit(this.mock, 'GRC7913ThresholdSet').withArgs(9);
      ...
      await expect(this.mock.$_setThreshold(10))
        .to.be.revertedWithCustomError(this.mock, 'MultiSignerGRC7913UnreachableThreshold')
        .withArgs(9, 10);
    });
    ...
    it('removing signers should not make threshold unreachable', async function () {
      ...
      await expect(this.mock.$_removeSigners([signer1]))
        .to.emit(this.mock, 'GRC7913SignerRemoved')
        .withArgs(signer1)
        .to.not.emit(this.mock, 'GRC7913SignerWeightChanged');
    });
