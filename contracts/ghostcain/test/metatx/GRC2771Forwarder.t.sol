...
import {GRC2771Forwarder} from "@ghostchain/contracts/metatx/GRC2771Forwarder.sol";
import {CallReceiverMockTrustingForwarder, CallReceiverMock} from "@ghostchain/contracts/mocks/CallReceiverMock.sol";
import {ECDSA} from "@ghostchain/contracts/utils/cryptography/ECDSA.sol";
import {SafeCast} from "@ghostchain/contracts/utils/math/SafeCast.sol";
...
contract GRC2771ForwarderMock is GRC2771Forwarder {
    constructor(string memory name) GRC2771Forwarder(name) {}

    function forwardRequestStructHash(
        GRC2771Forwarder.ForwardRequestData calldata request,
        uint256 nonce
...
contract GRC2771ForwarderTest is Test {
    using ECDSA for bytes32;

    GRC2771ForwarderMock internal _erc2771Forwarder;
    ...
    function setUp() public {
        _erc2771Forwarder = new GRC2771ForwarderMock("GRC2771Forwarder");
        _receiver = new CallReceiverMockTrustingForwarder(address(_erc2771Forwarder));
    }

    // Forge a new ForwardRequestData
    function _forgeRequestData() private view returns (GRC2771Forwarder.ForwardRequestData memory) {
        return
            _forgeRequestData({
                value: 0,
    ...
    function _forgeRequestData(
        ...
        uint48 deadline,
        bytes memory data
    ) private view returns (GRC2771Forwarder.ForwardRequestData memory) {
        return
            GRC2771Forwarder.ForwardRequestData({
                from: _signer,
    ...
    function _signRequestData(
        GRC2771Forwarder.ForwardRequestData memory request,
        uint256 nonce
    ) private view returns (GRC2771Forwarder.ForwardRequestData memory) {
    ...
    function _tamperRequestData(
        GRC2771Forwarder.ForwardRequestData memory request,
        TamperType tamper
    ) private returns (GRC2771Forwarder.ForwardRequestData memory) {
        ...
        if (tamper == TamperType.TO) {
            vm.expectRevert(
                abi.encodeWithSelector(
                    GRC2771Forwarder.GRC2771UntrustfulTarget.selector,
                    request.to,
            ...
            vm.expectRevert(
                abi.encodeWithSelector(GRC2771Forwarder.GRC2771ForwarderInvalidSigner.selector, recovered, request.from)
            );
        }
        return request;
    }
    ...
    function testExecuteAvoidsETHStuck(uint256 initialBalance, uint256 value, bool targetReverts) public {
        ...
        GRC2771Forwarder.ForwardRequestData memory request = _forgeRequestData({
            value: value,
            deadline: uint48(block.timestamp + 1),
            data: targetReverts
    ...
    function testExecuteBatchAvoidsETHStuck(uint256 initialBalance, uint256 batchSize, uint256 value) public {
        ...
        GRC2771Forwarder.ForwardRequestData[] memory requests = new GRC2771Forwarder.ForwardRequestData[](batchSize);
    ...
    function testExecuteBatchTamperedValuesZeroReceiver(uint8 _tamper) public {
        ...
        GRC2771Forwarder.ForwardRequestData[] memory requests = new GRC2771Forwarder.ForwardRequestData[](3);
