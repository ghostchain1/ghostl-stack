...
async function fixture() {
  ...
  const forwarder = await ethers.deployContract('GRC2771Forwarder', ['GRC2771Forwarder']);
...
describe('GRC2771Forwarder', function () {
  ...
  describe('execute', function () {
    ...
    describe('with tampered request', function () {
      it('reverts with valid signature for non-current nonce', async function () {
        ...
        // nonce has changed
        await expect(this.forwarder.execute(request))
          .to.be.revertedWithCustomError(this.forwarder, 'GRC2771ForwarderInvalidSigner')
          .withArgs(
      ...
      it('reverts with valid signature for expired deadline', async function () {
        ...
        await expect(this.forwarder.execute(request))
          .to.be.revertedWithCustomError(this.forwarder, 'GRC2771ForwarderExpiredRequest')
      ...
      it('reverts with valid signature but mismatched value', async function () {
        ...
        await expect(this.forwarder.execute(request))
          .to.be.revertedWithCustomError(this.forwarder, 'GRC2771ForwarderMismatchedValue')
  ...
  describe('executeBatch', function () {
    ...
    describe('with valid requests', function () {
      ...
      it('atomic batch with reverting request reverts the whole batch', async function () {
        ...
        ).to.be.revertedWithCustomError(this.forwarder, 'GRC2771ForwarderFailureInAtomicBatch');
      });
    });
