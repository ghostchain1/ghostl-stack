...
async function getStatus() {
    const url = `${(0, api_client_1.apiBaseUrl)().replace(/\/+$/, "")}/status`;
    try {
...
async function Page() {
    ...
    const lockContention = status?.lockContention;
    const rpcPreflight = status?.rpcPreflight;
    ...
    return (<div className="grid">
      <div className="card">
        ...
        {!lockContention ? (<p className="muted">Lock-contention metrics unavailable.</p>) : (<>
            <p style={{ marginTop: 8 }}>
            ...
            {recent.length > 0 ? (<>
                ...
                <table>
                  ...
                  <tbody>
                    {recent.map((row) => (<tr key={`${row.iteration ?? "na"}-${row.generatedAtUtc ?? "na"}`}>
      ...
      <div className="card">
        ...
        {!rpcPreflight ? (<p className="muted">RPC-preflight mitigation metrics unavailable.</p>) : (<>
            <p style={{ marginTop: 8 }}>
            ...
            {rpcRecent.length > 0 ? (<>
                ...
                <table>
                  ...
                  <tbody>
                    {rpcRecent.map((row) => (<tr key={`${row.trigger ?? "na"}-${row.generatedAtUtc ?? "na"}`}>
