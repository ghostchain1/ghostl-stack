...
if [[ -n "$BRIDGE_ADDR" ]]; then
  relayer_call_data="$(node -e 'const {ethers}=require("ethers"); const i=new ethers.Interface(["function relayer() view returns (address)"]); process.stdout.write(i.encodeFunctionData("relayer"));')"
  ...
  finalize_call_data="$(node -e 'const {ethers}=require("ethers"); const i=new ethers.Interface(["function finalizeToL3(address,address,uint256,uint256)"]); process.stdout.write(i.encodeFunctionData("finalizeToL3", ["0x70997970C51812dc3A010C7d01b50e0d17dc79C8","0x70997970C51812dc3A010C7d01b50e0d17dc79C8",1,1]));')"
