# NOTE: This is a lightly patched copy of:
#   infra/opstack/optimism-upstream/ops/docker/op-stack-go/Dockerfile
#
# We keep it outside the submodule so we can patch build behavior without
# mutating the upstream submodule checkout.
#
# Patch rationale:
# - optimism-upstream/go.mod contains a local replace directive:
#     replace github.com/mholt/archiver => ./third_party/archiver
# - The upstream Dockerfile runs `go mod download` after copying only go.mod/go.sum
#   which fails because the replaced module's go.mod is not present yet.
# - Copy the replaced module's go.mod/go.sum early to make the warm-up step work.
...
# Local replace directives require the replaced module go.mod to exist during `go mod download`.
COPY ./third_party/archiver/go.mod /app/third_party/archiver/go.mod
COPY ./third_party/archiver/go.sum /app/third_party/archiver/go.sum
...
# Some upstream checkouts have a `go` directive set to a future patch version (e.g. 1.24.12),
# which breaks builds when the toolchain is pinned (GOTOOLCHAIN=local). Normalize to 1.24.0.
RUN sed -i 's/^go .*/go 1.24.0/' /app/third_party/archiver/go.mod

# github.com/crate-crypto/go-kzg-4844 v1.0.0 is incompatible with newer gnark-crypto versions.
# Bump it to a compatible version for deterministic Docker builds.
RUN go mod edit -require=github.com/crate-crypto/go-kzg-4844@v1.1.0

# libp2p v0.36.x expects quic-go APIs that were removed in newer quic-go releases.
RUN go mod edit -replace=github.com/quic-go/quic-go=github.com/quic-go/quic-go@v0.45.2
RUN go mod edit -replace=github.com/quic-go/qpack=github.com/quic-go/qpack@v0.4.0
...
# Normalize again after copying the full context (COPY . overwrites the earlier copy).
RUN sed -i 's/^go .*/go 1.24.0/' /app/third_party/archiver/go.mod

# COPY . overwrites go.mod/go.sum; re-apply and ensure sums are present for subsequent builds.
RUN go mod edit -require=github.com/crate-crypto/go-kzg-4844@v1.1.0
RUN go mod edit -replace=github.com/quic-go/quic-go=github.com/quic-go/quic-go@v0.45.2
RUN go mod edit -replace=github.com/quic-go/qpack=github.com/quic-go/qpack@v0.4.0
RUN --mount=type=cache,target=/go/pkg/mod --mount=type=cache,target=/root/.cache/go-build go mod download all
