...
ENSURE_OUT="$(L1_TOKEN_ADDRESS="$L1_TOKEN_ADDRESS" L2_TOKEN_ADDRESS="$L2_TOKEN_ADDRESS" \
...
perl -0777 -i -pe "s/^L2_TOKEN_ADDRESS=.*$/L2_TOKEN_ADDRESS=$L2_TOKEN_ADDRESS/m" "$STACK_ENV"
...
DEMO_ACCOUNT="$DEMO_ACCOUNT" L2_TOKEN_ADDRESS="$L2_TOKEN_ADDRESS" \
  OP_L2_RPC="http://localhost:29547" \
  npx hardhat run --network ghostl2Op --no-compile scripts/demo_balance_l2.ts
