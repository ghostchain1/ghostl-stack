...
usage() {
  cat <<'EOF'
Usage: infra/scripts/evidence-pack-l3.sh [--verify] [--out-dir=PATH]
...
copy_l3_configs() {
  local dest_root="$1"
  local l3_root="${ROOT_DIR}/infra/opstack/l3"
  if [ ! -d "$l3_root" ]; then
    return 0
  fi
  shopt -s nullglob
  local config_dir
  for config_dir in "$l3_root"/*/config; do
    local rel="${config_dir#$ROOT_DIR/}"
    mkdir -p "${dest_root}/$(dirname "$rel")"
    copy_dir "$config_dir" "${dest_root}/${rel}"
  done
  shopt -u nullglob
}
...
build_pack() {
  ...
  local snapshots=(
    ...
    "docs/l3/architecture-current.md"
    "docs/l3/l2-alignment-audit.md"
    "docs/l3/reliability.md"
    "docs/l3/observability.md"
    "docs/l3/governance-autonomy.md"
    "docs/l3/secrets-model.md"
    "docs/opstack-l2-l3-stack.md"
  )
  ...
  copy_l3_configs "${work_dir}/snapshots"
  ...
  {
    echo "# L3 config snapshots"
    hash_dir "${work_dir}/snapshots"
  } > "${work_dir}/hashes/config.sha256"

  {
    echo "# L3 contract sources"
    hash_dir "${ROOT_DIR}/contracts/src/l3"
...
{
  "schemaVersion": "1.0",
  "buildType": "ghostchain.l3.evidence-pack",
  ...
  "builder": {
    "tool": "infra/scripts/evidence-pack-l3.sh",
    "version": "1"
  }
}
EOF
  ...
  local pack_dir="${out_dir}/evidence-pack-l3-${timestamp}"
  rm -rf "$pack_dir"
  mkdir -p "$pack_dir"
  ...
  local zip_path="${out_dir}/evidence-pack-l3-${timestamp}.zip"
