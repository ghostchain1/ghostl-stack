...
cd "$OP_DIR"
docker compose "${COMPOSE_ENV_ARGS[@]}" stop l1 l2-geth op-node op-batcher op-proposer >/dev/null 2>&1 || true
docker compose "${COMPOSE_ENV_ARGS[@]}" rm -f l1 l2-geth op-node op-batcher op-proposer >/dev/null 2>&1 || true
