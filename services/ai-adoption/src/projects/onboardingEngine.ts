...
export async function onboardProject(project: Web3Project): Promise<OnboardingPlan> {
  const plan: OnboardingPlan = {
    ...
    steps: [
      ...
      `2. Configure RPC: https://rpc.ghostchain.cloud (chain-id: 14000101)`,
      `3. Bridge existing ${project.currentChain} assets via bridge.ghostchain.cloud`,
      `4. Deploy contracts: npx hardhat deploy --network ghostchain`,
      `5. Register on GhostXchange for liquidity incentives`,
      `6. Submit grant application at dev.ghostchain.cloud/grants`,
    ],
    bridgeUrl:     "https://bridge.ghostchain.cloud",
    docsUrl:       "https://docs.ghostchain.cloud",
