...
interface VaultError extends Error { status?: number; }
...
async function vaultFetch(
  ...
  if (!res.ok) {
    const err: VaultError = new Error(`vault ${res.status}: ${text.slice(0, 200)}`);
...
/** Read a KV v2 secret */
export async function kvRead(mount: string, secretPath: string): Promise<Record<string, string>> {
  await ensureToken();
  const res = await vaultFetch('GET', `/v1/${mount}/data/${secretPath.replace(/^\//, '')}`) as {
...
export async function kvWrite(mount: string, secretPath: string, data: Record<string, string>): Promise<void> {
  await ensureToken();
  await vaultFetch('POST', `/v1/${mount}/data/${secretPath.replace(/^\//, '')}`, { data });
}

/** Read a KV v1 secret */
export async function kvReadV1(mount: string, secretPath: string): Promise<Record<string, string>> {
  await ensureToken();
  const res = await vaultFetch('GET', `/v1/${mount}/${secretPath.replace(/^\//, '')}`) as {
...
export async function rotateSecret(rule: {
  mount: string;
  path: string;
  kvVersion?: number;
  keys?: string[];
  encoding?: 'base64' | 'hex';
  keyLength?: number;
}): Promise<{ ok: boolean; rotated?: string[]; reason?: string }> {
  ...
  try {
    ...
    for (const k of keys) {
      ...
      logger.registerSecret(updated[k]); // redact new value in logs
    }
