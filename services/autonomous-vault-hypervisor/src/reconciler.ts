...
import { publishHealthSignal, publishAnomalySignal, publishMemoryPressureSignal } from './ghostbrain.js';
import { sampleMemoryPressure, runMemorySwapCycle } from './memory-swap-manager.js';
import type { ReconcileState, VmInfo, ContainerInfo } from './types.js';

export const state: ReconcileState = {
...
async function remediatePhase(): Promise<void> {
  ...
  if (executed > 0) {
    logger.info('Remediation phase complete', { executed });
  }
}

async function memorySwapPhase(): Promise<void> {
  try {
    const signals = await sampleMemoryPressure();

    // Publish all signals to GhostBrain Core for the advisor to process
    for (const sig of signals) {
      publishMemoryPressureSignal(sig);
      if (sig.anomaly) {
        publishAnomalySignal('memory.pressure', sig.pressureRatio, 0.80);
      }
    }

    // Also run a local swap cycle for immediate relief when
    // GhostBrain advisor is unavailable (standalone mode)
    const hostSignal = signals.find(s => s.source === 'host');
    if (hostSignal?.anomaly && hostSignal.profiles.length > 0) {
      const outcomes = await runMemorySwapCycle(hostSignal.profiles);
      const reclaimed = outcomes.filter(o => o.success).reduce((s, o) => s + o.reclaimedMiB, 0);
      if (reclaimed > 0) {
        logger.info('MemorySwap local cycle reclaimed', { reclaimedMiB: reclaimed });
      }
    }
  } catch (err) {
    logger.warn('Memory swap phase error', { err: String(err) });
  }
}
...
async function runOnce(): Promise<void> {
  ...
  try {
    ...
    // Publish health signal to GhostBrain
    publishHealthSignal({
      signalId:   randomUUID(),
      source:     'manual',
