...
// Track per-target remediation counts within a rolling window (1 hour)
...
export async function remediateVms(state: ReconcilerState, executed: number): Promise<number> {
  ...
  for (const vm of state.vms.values()) {
    ...
    if (!gate.allowed) {
      ...
      const evt = { ts: Date.now(), type: 'vm_start', target: vm.name, reason: `state:${vm.state}`, outcome: 'policy_denied' as const };
    ...
    const result = vm.state === 'crashed' ? await restartVm(vm.name) : await startVm(vm.name);
    const outcome = result.ok ? 'success' as const : 'failed' as const;
    const evt = {
      ts: Date.now(),
      type: vm.state === 'crashed' ? 'vm_restart' as const : 'vm_start' as const,
      target: vm.name,
...
export async function remediateContainers(state: ReconcilerState, executed: number): Promise<number> {
  if (!CFG.remediateEnabled) return executed;
  ...
  for (const ct of state.containers.values()) {
    ...
    if (!gate.allowed) {
      ...
      const evt = { ts: Date.now(), type: 'container_start' as const, target: ct.name, reason: `state:${ct.state}`, outcome: 'policy_denied' as const };
      state.remediations.push(evt);
      if (state.remediations.length > 500) state.remediations.shift();
      continue;
    }
    ...
    let evtType: 'container_start' | 'container_restart';
