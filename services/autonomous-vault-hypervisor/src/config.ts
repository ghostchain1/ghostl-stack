// config.ts — centralised environment-driven configuration
...
export const CFG = {
  ...
  ghostbrainEnabled: (process.env.GHOSTBRAIN_ENABLED ?? '1') !== '0' && process.env.GHOSTBRAIN_ENABLED !== 'false',
  ...
  // Secret rotation
  rotateIntervalMs: Number(process.env.ROTATE_INTERVAL_MS ?? 900_000), // 15 min default
  ...
  // Layer ownership mappings (json map: vmName -> layer)
  vmLayerMap: (() => {
    try { return JSON.parse(process.env.VM_LAYER_MAP ?? '{}') as Record<string, string>; }
  ...
  // Log redaction — per AGENTS.md §6
  redactFields: ['VAULT_TOKEN', 'VAULT_SECRET_ID', 'HMAC_SECRET', 'HYPERVISOR_SSH_KEY_PATH', 'password', 'token', 'secret'],
} as const;
