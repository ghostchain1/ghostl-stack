...
import type { GhostBrainEnvelope, HealthSignal, Layer, MemoryPressureSignal, SwapDirective, SwapOutcome } from './types.js';

const AGENT_ID   = CFG.serviceName;
const AGENT_ROLE = 'hypervisor';
...
function _envelope<T>(subject: string, payload: T): string {
  const env: GhostBrainEnvelope<T> = {
  ...
  try {
    ...
    void (async () => {
      for await (const m of swapSub) {
        try {
          const envelope = JSON.parse(sc.decode(m.data)) as GhostBrainEnvelope<SwapDirective>;
          ...
          const outcome: SwapOutcome = await handleSwapDirective(directive);
...
export function publishMemoryPressureSignal(signal: MemoryPressureSignal): void {
...
export function publishAnomalySignal(metric: string, value: number, threshold: number): void {
  ...
  // Also send to GhostBrain HTTP if available (fire-and-forget)
  if (CFG.ghostbrainEnabled) {
    fetch(`${CFG.ghostbrainUrl}/api/v1/signals`, {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({ source: 'manual', service: AGENT_ID, layer: 'L1', anomaly: true, metric, value, threshold, observedAt: new Date().toISOString() }),
...
export async function drainNats(): Promise<void> {
  if (_nc) { try { await _nc.drain(); } catch { /* ignore */ } }
}
