...
import { discoverVms, startVm, shutdownVm, destroyVm, restartVm, snapshotVm, domainInfo, listSnapshots } from './vm-manager.js';
import { discoverContainers, restartContainer, startContainer, stopContainer, inspectContainer, containerLogs, containerStats, pruneContainers, dockerAvailable } from './docker-manager.js';
...
export function createApp(): express.Application {
  ...
  app.post('/v1/vms/:name/start', async (req, res) => {
    ...
    if (!gate.allowed) {
      return res.status(403).json({ ok: false, reason: gate.reason });
    }
    try {
  ...
  app.post('/v1/vms/:name/snapshot', async (req, res) => {
    ...
    try {
      const snapName = (req.body as { name?: string }).name;
  ...
  app.post('/v1/secrets/rotate', async (req, res) => {
    const gate = checkAction('secret.rotate', (req.body as { path?: string }).path ?? '*');
    if (!gate.allowed) return res.status(403).json({ ok: false, reason: gate.reason });
    try {
      const rule = req.body as {
        mount: string; path: string; kvVersion?: 1 | 2;
        keys?: string[]; encoding?: 'base64' | 'hex';
      };
  ...
  app.get('/v1/reconcile/state', (_req, res) => {
    res.json({
      ...
      lastContainerDiscovery: state.lastContainerDiscovery ? new Date(state.lastContainerDiscovery).toISOString() : null,
      vms: state.vms.size,
      containers: state.containers.size,
    });
  });
  ...
  app.get('/v1/reconcile/remediations', (_req, res) => {
    const limit = Math.min(Number(((_req.query as Record<string, string>).limit) ?? 100), 500);
