...
_REGISTER_BODY = json.dumps({
    "agentId":      AGENT_ID,
    "role":         AGENT_ROLE,
    "capabilities": ["network.dns.update", "network.tls.renew", "metrics.query"],
    "resourceScopes": [
        {"type": "network", "name": "ghostchain.cloud", "layer": "L1"},
        {"type": "network", "name": "ghostchain.cloud", "layer": "L2"},
        {"type": "stack",   "name": "ghostdns-ai",       "layer": "L2"},
    ],
...
def ghostbrain_register(retries: int = 5, delay_s: float = 3.0) -> None:
    ...
    for attempt in range(1, retries + 1):
        ...
        if ok:
            logger.info("[ghostbrain] Registered with GhostBrain Core", extra={"role": AGENT_ROLE, "url": _GHOSTBRAIN_URL})
            return
