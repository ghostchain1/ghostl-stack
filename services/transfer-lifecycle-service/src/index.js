...
app.get("/health", (_req, res) => res.json({ ok: true, service: "transfer-lifecycle-service" }));
...
app.get("/transfers", async (_req, res) => {
  try {
    ...
    res.json({
      ok: true,
      transfers: [
        { id: "t-latest", srcChain: "l2", dstChain: "l3", status: Number(pending) > 0 ? "pending" : "finalized", amount: "0", txs: [] }
      ],
      stats: { pending, finalized }
    });
  } catch (e) {
