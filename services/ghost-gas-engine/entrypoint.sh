...
MIGRATION_TIMEOUT_MS="${MIGRATION_TIMEOUT_MS:-120000}"
READINESS_FILE="${READINESS_FILE:-/tmp/ghost_gas_engine_migrated}"

if [ "$RUN_MIGRATIONS" = "1" ]; then
  echo "Phase 1: migrate/seed"
  if [ -z "${MIGRATION_COMMAND:-}" ]; then
    if [ -f dist/db/seed.js ]; then
      MIGRATION_COMMAND="node dist/db/seed.js"
    else
      MIGRATION_COMMAND="npm run migrate"
    fi
  fi
  export MIGRATION_COMMAND
  export MIGRATION_TIMEOUT_MS

  if node -e "
    const { spawn } = require('child_process');
    const cmd = process.env.MIGRATION_COMMAND;
    const ms = Number(process.env.MIGRATION_TIMEOUT_MS || 120000);
    const p = spawn(cmd, { stdio: 'inherit', shell: true });
    const t = setTimeout(() => {
      console.error('Migration watchdog timeout');
      p.kill('SIGKILL');
      process.exit(1);
    }, ms);
    if (t.unref) t.unref();
