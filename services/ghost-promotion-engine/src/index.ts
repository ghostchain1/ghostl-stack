 ...
 *   RPC_L2_URL              L2 testnet RPC                  (default: http://10.50.99.77:29545)
...
const RPC_TESTNET_L2 = (process.env.RPC_L2_URL      ?? 'http://10.50.99.77:29545').replace(/\/$/, '');
...
// ── RPC health check — ghost_ namespace ───────────────────────────────────────
async function checkRpc(label: string, url: string): Promise<GateResult> {
  try {
    const body = JSON.stringify({
      jsonrpc: '2.0',
      method:  'ghost_blockNumber',   // GhostChain RPC — never eth_
      params:  [],
      id:      1,
    });
    const res  = await fetch(url, {
    ...
    return { passed: ok, details: ok ? `block: ${json.result}` : 'no result' };
  } catch (err) {
    return { passed: false, details: err instanceof Error ? err.message : String(err) };
  }
}
...
async function gateMainnetRpc(): Promise<GateResult> {
  return checkRpc('mainnet-l1', RPC_MAINNET_L1);
}
...
async function runPromotionPipeline(): Promise<void> {
  ...
  try {
    // ──────────────────────────────────────────────────────────────────────────
    // PHASE A — Testnet RPC gate
    // ──────────────────────────────────────────────────────────────────────────
    log('INFO', '🔍 Phase A: Testnet RPC health gate');
    ...
    // ──────────────────────────────────────────────────────────────────────────
    // PHASE B — AI Simulation on testnet
    // ──────────────────────────────────────────────────────────────────────────
    ...
    log('INFO', '🧪 Phase B: AI simulation on testnet');
    ...
    // ──────────────────────────────────────────────────────────────────────────
    // PHASE C — Security audit gate
    // ──────────────────────────────────────────────────────────────────────────
    state.stage = 'SECURITY_AUDIT';
    await saveState();

    log('INFO', '🔐 Phase C: Security audit gate');
    ...
    // ──────────────────────────────────────────────────────────────────────────
    // PHASE D — Submit governance proposal for mainnet promotion
    // ──────────────────────────────────────────────────────────────────────────
    state.stage = 'GOVERNANCE_PENDING';
    await saveState();

    log('INFO', '👑 Phase D: Submitting mainnet promotion proposal to governance');
    ...
    // ──────────────────────────────────────────────────────────────────────────
    // PHASE E — GAIS mainnet VM sync (advisory — GAIS enforces its own safety)
    // ──────────────────────────────────────────────────────────────────────────
    state.stage = 'MAINNET_PROMOTING';
    await saveState();

    log('INFO', '🚀 Phase E: Requesting GAIS mainnet VM health check');
    ...
    // Submit advisory restart proposal for mainnet VMs that are unhealthy
    ...
    if (unhealthy.length > 0) {
      await submitProposal('restart_unhealthy_mainnet_vms', {
        ...
        reason: 'Post-promotion health sync',
      });
    }
    ...
    // ──────────────────────────────────────────────────────────────────────────
    // PHASE F — Mainnet RPC verification
    // ──────────────────────────────────────────────────────────────────────────
    log('INFO', '✅ Phase F: Verifying mainnet L1 RPC');
    const mainnetRpc = await gateMainnetRpc();
    log('INFO', 'Mainnet L1 RPC gate result', mainnetRpc);

    if (!mainnetRpc.passed) {
      log('WARN', 'Mainnet L1 RPC unhealthy post-promotion — submitting heal proposal');
      await submitProposal('heal_mainnet_l1', {
        details: mainnetRpc.details,
        vm:      'ghostchain-mainnet-l1',
      });
    }

    // ──────────────────────────────────────────────────────────────────────────
    // PHASE G — App deployment proposal
    // ──────────────────────────────────────────────────────────────────────────
    state.stage = 'DEPLOYING_APPS';
    await saveState();

    log('INFO', '🌐 Phase G: Submitting app deployment proposal');
    await submitProposal('deploy_apps', {
      gitHash:  state.currentGitHash,
      targets:  ['ghost-web', 'apps/web', 'apps/api'],
      vm:       'ghost-web',
      chainId:  CHAIN_L3,    // Apps use L3 for transactions per architecture
    });
...
async function main(): Promise<void> {
  ...
  if (state.stage === 'SIMULATING_TESTNET' || state.stage === 'DEVNET_READY') {
