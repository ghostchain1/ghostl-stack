...
# Blockscout expects legacy env var names. Keep operator-facing config GST-native.
GHOSTSCOUT_UPSTREAM_HTTP_URL="${GHOSTSCOUT_UPSTREAM_HTTP_URL:-${GST_JSONRPC_HTTP_URL:-}}"
GHOSTSCOUT_UPSTREAM_TRACE_URL="${GHOSTSCOUT_UPSTREAM_TRACE_URL:-${GST_JSONRPC_TRACE_URL:-}}"
GHOSTSCOUT_UPSTREAM_WS_URL="${GHOSTSCOUT_UPSTREAM_WS_URL:-${GST_JSONRPC_WS_URL:-}}"

if [ -n "${GHOSTSCOUT_UPSTREAM_HTTP_URL:-}" ] && [ -z "${ETHEREUM_JSONRPC_HTTP_URL:-}" ]; then
  export ETHEREUM_JSONRPC_HTTP_URL="${GHOSTSCOUT_UPSTREAM_HTTP_URL}"
fi
if [ -n "${GHOSTSCOUT_UPSTREAM_TRACE_URL:-}" ] && [ -z "${ETHEREUM_JSONRPC_TRACE_URL:-}" ]; then
  export ETHEREUM_JSONRPC_TRACE_URL="${GHOSTSCOUT_UPSTREAM_TRACE_URL}"
fi
if [ -n "${GHOSTSCOUT_UPSTREAM_WS_URL:-}" ] && [ -z "${ETHEREUM_JSONRPC_WS_URL:-}" ]; then
  export ETHEREUM_JSONRPC_WS_URL="${GHOSTSCOUT_UPSTREAM_WS_URL}"
fi
