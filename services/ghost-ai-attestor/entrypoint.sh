...
resolve_node_cmd() {
  local candidate
  for candidate in dist/server.js dist/index.js index.mjs index.js src/server.js src/index.js; do
    if [ -f "$candidate" ]; then
  ...
  if [ -f src/server.ts ] && [ -d node_modules/ts-node ]; then
    NODE_CMD=(node --loader ts-node/esm --no-warnings --experimental-specifier-resolution=node src/server.ts)
    return 0
  fi
  if [ -f /app/src/server.ts ] && [ -d /app/node_modules/ts-node ]; then
    NODE_CMD=(node --loader ts-node/esm --no-warnings --experimental-specifier-resolution=node /app/src/server.ts)
    return 0
  fi
